...
const hueExtend = {
  ...
  light_onoff_brightness_color: (options = {}) => ({
    ...extendDontUse.light_onoff_brightness_color({
      supportsHS: true,
      ...options,
    }),
    ota: ota.zigbeeOTA,
    meta: { turnsOffAtBrightness1: true },
    toZigbee: extendDontUse
      .light_onoff_brightness_color({ supportsHS: true, ...options })
      .toZigbee.concat([tz.hue_power_on_behavior, tz.hue_power_on_error]),
  }),
  light_onoff_brightness_colortemp_color: (options = {}) => ({
    ...extendDontUse.light_onoff_brightness_colortemp_color({
      supportsHS: true,
      ...options,
    }),
    ota: ota.zigbeeOTA,
    meta: { turnsOffAtBrightness1: true },
    toZigbee: extendDontUse
      .light_onoff_brightness_colortemp_color({ supportsHS: true, ...options })
      .toZigbee.concat([tz.hue_power_on_behavior, tz.hue_power_on_error]),
  }),
      ...
      .exposes.concat([
        ...
        exposes
          .list(
            "gradient",
            ea.ALL,
            exposes.text("hex", "Color in RGB HEX format (eg #663399)")
          )
          .withLengthMin(1)
          .withLengthMax(9)
          .withDescription("List of RGB HEX colors"),
      ]),
    fromZigbee: extendDontUse
      .light_onoff_brightness_colortemp_color({ supportsHS: true, ...options })
      .fromZigbee.concat([fzLocal.gradient({ reverse: true })]),
    toZigbee: extendDontUse
      .light_onoff_brightness_colortemp_color({ supportsHS: true, ...options })
      .toZigbee.concat([
  ...
  hue_tap_dial: {
    ...
    convert: (model, msg, publish, options, meta) => {
      ...
      if (button === "dial") {
        ...
        const speed =
          adjustedTime <= 25 ? "step" : adjustedTime <= 75 ? "slow" : "fast";
        ...
        if (options.simulated_brightness) {
          ...
          const deltaOpts =
            typeof opts === "object" && opts.hasOwnProperty("delta")
              ? opts.delta
              : 35;
          const delta = direction === "right" ? deltaOpts : deltaOpts * -1;
          const brightness =
            globalStore.getValue(msg.endpoint, "brightness", 255) + delta;
        ...
        if (type === "press")
          ...
          payload.action_duration =
            (Date.now() - globalStore.getValue(msg.endpoint, "press_start")) /
            1000;
        }
      }
      return payload;
...
module.exports = [
  ...
  },
  {
    ...
    extend: hueExtend.light_onoff_brightness_colortemp_color_gradient({
      colorTempRange: [153, 500],
    }),
  },
  {
  ...
  },
  {
    ...
    exposes: [
      ...
      exposes.enum("device_mode", ea.ALL, [
        "single_rocker",
        "single_push_button",
        "dual_rocker",
        "dual_push_button",
  ...
  },
  {
    ...
    toZigbee: [
      ...
      tz.hue_motion_sensitivity,
      tz.hue_motion_led_indication,
    ],
    endpoint: (device) => {
  ...
  },
  {
    ...
    exposes: [
      ...
      e.illuminance_lux(),
      e.illuminance(),
      exposes.enum("motion_sensitivity", ea.ALL, [
        "low",
        "medium",
        "high",
        "very_high",
