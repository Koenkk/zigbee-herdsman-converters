...
module.exports = [
    ...
    fromZigbee: [fz.temperature, fz.illuminance, fz.humidity, fz.occupancy, fz.sprut_occupancy, fz.co2, fz.sprut_voc, fz.sprut_noise, fz.sprut_noise_detected, fz.on_off, fz.sw_version],
    toZigbee: [tz.on_off],
    exposes: [e.temperature().withEndpoint(1), e.illuminance(), e.illuminance_lux(), e.humidity(), e.occupancy(), e.occupancy_level(), e.co2(), e.voc(), e.noise(), e.noise_detected(), e.switch().withEndpoint(2), e.switch().withEndpoint(3), e.switch().withEndpoint(4)],
    configure: async (device, coordinatorEndpoint, logger) => {
        const endpoint1 = device.getEndpoint(1);
        const binds = ['genBasic','msTemperatureMeasurement', 'msIlluminanceMeasurement', 'msRelativeHumidity', 'msOccupancySensing', 'msCO2', 'sprutDevice', 'sprutVoc', 'sprutNoise', 'sprutIrBlaster'];
