...
async function assertDefintion(args: {
    ...
    bind: string[],
    read: [string, string[]][],
    configureReporting: [string, ReturnType<typeof reportingItem>[]][]
}) {
    const coordinatorEndpoint = mockEndpoint();
    const endpoint = args.device.endpoints[0];
    ...
    const expectedExposes = definition.exposes?.map((e) => e.name ?? `${e.type}(${e.features?.map((f) => f.name).join(',')})`).sort();
    logIfNotEqual(expectedExposes, args.exposes);
    expect(expectedExposes).toEqual(args.exposes);

    expect(endpoint.bind).toHaveBeenCalledTimes(args.bind.length);
    args.bind.forEach((bind, idx) => expect(endpoint.bind).toHaveBeenNthCalledWith(idx + 1, bind, coordinatorEndpoint));

    expect(endpoint.read).toHaveBeenCalledTimes(args.read.length);
    args.read.forEach((read, idx) => expect(endpoint.read).toHaveBeenNthCalledWith(idx + 1, read[0], read[1]));

    expect(endpoint.configureReporting).toHaveBeenCalledTimes(args.configureReporting.length);
    args.configureReporting.forEach((configureReporting, idx) => expect(endpoint.configureReporting).toHaveBeenNthCalledWith(idx + 1, configureReporting[0], configureReporting[1]));
}
...
describe('ModernExtend', () => {
    ...
    test('onOff({powerOnBehavior: false}), electricalMeasurements({current: {divisor: 1000}, voltage: {divisor: 1}, power: {divisor: 1}, energy: {divisor: 100}})', async () => {
        await assertDefintion({
            ...
            bind: ['genOnOff', 'haElectricalMeasurement', 'seMetering'],
            read: [
                ['genOnOff', ['onOff']],
    ...
    test(`philipsLight({gradient: {extraEffects: ['sparkle', 'opal', 'glisten']}, colorTemp: {range: [153, 500]}})`, async () => {
        await assertDefintion({
            ...
            exposes: ['effect', 'effect', 'gradient', 'gradient_scene', 'light(state,brightness,color_temp,color_temp_startup,color_xy,color_hs)', 'linkquality', 'power_on_behavior'],
