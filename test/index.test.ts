...
describe("ZHC", () => {
    ...
    it("decodes the HPZERAD-V1 manufacturer attributes", () => {
        ...
        const result = converter.convert(
            ...
            {
                data: {
                    heiwaDisplayTemperature: 247,
                    heiwaDisplayedSetpoint: 220,
                    heiwaDisplayBrightness: 25,
                    heiwaDisplayTemperatureSource: 2,
                    heiwaSetpointOnlyUi: 1,
                    heiwaActiveProfile: 2,
                    heiwaHeatingDemandIcon: 0,
                    heiwaCoolingDemandIcon: 1,
                },
                device,
            } as never,
            () => {},
        ...
        expect(result).toStrictEqual({
            ...
            display_temperature: 24.7,
            current_heating_setpoint: 22,
            display_brightness: 5,
            display_temperature_source: "temperature_2",
            setpoint_only_ui: "ON",
            profile: "reduced",
            active_profile: "reduced",
            remote_power: "ON",
            zone_demand_icon: "cooling",
        });
    });

    it("writes the HPZERAD-V1 setpoint as coherent standard bounds", async () => {
        const device = mockDevice({modelID: "test", endpoints: [{ID: 25}]});
        const definition = baseDefinitions.find((candidate) => candidate.model === "HPZERAD-V1");
        ...
        device.endpoints[0].read.mockResolvedValueOnce({heiwaDisplayedSetpoint: 220});
        ...
        expect(device.endpoints[0].write).toHaveBeenCalledWith("hvacThermostat", {
            occupiedCoolingSetpoint: 2050,
            occupiedHeatingSetpoint: 2350,
        });
        expect(device.endpoints[0].read).toHaveBeenCalledWith("hvacThermostat", ["heiwaDisplayedSetpoint"]);
    });
    ...
    it("writes and verifies the HPZERAD-V1 stock profile request", async () => {
        const device = mockDevice({modelID: "test", endpoints: [{ID: 25}]});
        const definition = baseDefinitions.find((candidate) => candidate.model === "HPZERAD-V1");
        const converter = definition?.toZigbee?.find((candidate) => candidate.key?.includes("profile"));
        if (!definition || !converter?.convertSet) throw new Error("HPZERAD-V1 profile converter is missing");
        device.endpoints[0].read.mockResolvedValueOnce({heiwaActiveProfile: 1});

        const result = await converter.convertSet(device.endpoints[0], "profile", "eco", {} as never);

        expect(device.endpoints[0].write).toHaveBeenCalledWith("hvacThermostat", {heiwaProfileRequest: 1});
        expect(device.endpoints[0].write).toHaveBeenCalledTimes(1);
        expect(device.endpoints[0].read).toHaveBeenCalledWith("hvacThermostat", ["heiwaActiveProfile"]);
        expect(result).toStrictEqual({state: {profile: "eco", active_profile: "eco", remote_power: "ON"}});
    });

    it("writes the HPZERAD-V1 setpoint-only local interface setting", async () => {
        const device = mockDevice({modelID: "test", endpoints: [{ID: 25}]});
        const definition = baseDefinitions.find((candidate) => candidate.model === "HPZERAD-V1");
        const converter = definition?.toZigbee?.find((candidate) => candidate.key?.includes("setpoint_only_ui"));
        if (!definition || !converter?.convertSet) throw new Error("HPZERAD-V1 display settings converter is missing");

        await converter.convertSet(device.endpoints[0], "setpoint_only_ui", "ON", {} as never);

        expect(device.endpoints[0].write).toHaveBeenCalledWith("hvacThermostat", {heiwaSetpointOnlyUi: 1});
        expect(device.endpoints[0].read).toHaveBeenCalledWith("hvacThermostat", ["heiwaSetpointOnlyUi"]);
    });

    it("writes the HPZERAD-V1 demand icon attributes in separate transactions", async () => {
        const device = mockDevice({modelID: "test", endpoints: [{ID: 25}]});
        const definition = baseDefinitions.find((candidate) => candidate.model === "HPZERAD-V1");
        const converter = definition?.toZigbee?.find((candidate) => candidate.key?.includes("zone_demand_icon"));
        if (!definition || !converter?.convertSet) throw new Error("HPZERAD-V1 zone demand converter is missing");
        device.endpoints[0].read.mockResolvedValueOnce({heiwaHeatingDemandIcon: 1, heiwaCoolingDemandIcon: 0});

        await converter.convertSet(device.endpoints[0], "zone_demand_icon", "heating", {} as never);

        expect(device.endpoints[0].write).toHaveBeenNthCalledWith(1, "hvacThermostat", {heiwaCoolingDemandIcon: 0});
        expect(device.endpoints[0].write).toHaveBeenNthCalledWith(2, "hvacThermostat", {heiwaHeatingDemandIcon: 1});
    });

    it("rejects an unconfirmed HPZERAD-V1 demand icon change", async () => {
        const device = mockDevice({modelID: "test", endpoints: [{ID: 25}]});
        const definition = baseDefinitions.find((candidate) => candidate.model === "HPZERAD-V1");
        const converter = definition?.toZigbee?.find((candidate) => candidate.key?.includes("zone_demand_icon"));
        if (!definition || !converter?.convertSet) throw new Error("HPZERAD-V1 zone demand converter is missing");
        device.endpoints[0].read.mockResolvedValueOnce({heiwaHeatingDemandIcon: 0, heiwaCoolingDemandIcon: 0});

        await expect(converter.convertSet(device.endpoints[0], "zone_demand_icon", "cooling", {} as never)).rejects.toThrow(
            "Zone demand icon cooling was not confirmed by the thermostat",
        );
    });
