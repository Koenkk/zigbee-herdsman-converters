...
const devices = [
    ...
    // Meazon Plug and DinRail meters
    {
        ...
        configure: (ieeeAddr, shepherd, coordinator, callback) => {
            ...
            const linefrequency = {direction: 0, attrId: 0x2000, dataType: 0x29, minRepIntval: 0x0001, maxRepIntval: 300, repChange: 1};
            const actions = [
                (cb) => device.bind('genOnOff', coordinator, cb),
                (cb) => device.foundation('genOnOff', 'configReport', [onOff], foundationCfg, cb),
                ...
                (cb) => device.foundation('seMetering', 'configReport', [linefrequency], {manufSpec: 1, disDefaultRsp: 0, manufCode: 4406}, cb),
            ];
