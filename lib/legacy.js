...
const fromZigbee = {
    ...
    thermostat_att_report: {
        ...
        convert: (model, msg, publish, options, meta) => {
            ...
            if (typeof ctrl == 'number' && common.thermostatControlSequenceOfOperationsLegacy.hasOwnProperty(ctrl)) {
                result[postfixWithEndpointName('control_sequence_of_operation', msg, model)] =
                    common.thermostatControlSequenceOfOperationsLegacy[ctrl];
            }
            const smode = msg.data['systemMode'];
            if (typeof smode == 'number' && common.thermostatSystemModesLegacy.hasOwnProperty(smode)) {
                result[postfixWithEndpointName('system_mode', msg, model)] = common.thermostatSystemModesLegacy[smode];
            }
            const rmode = msg.data['runningMode'];
            if (typeof rmode == 'number' && common.thermostatSystemModesLegacy.hasOwnProperty(rmode)) {
                result[postfixWithEndpointName('running_mode', msg, model)] = common.thermostatSystemModesLegacy[rmode];
            }
        ...
        convert: (model, msg, publish, options, meta) => {
            ...
            if (mode == 'number') {
                ...
                case 5:
                    ...
                    result.system_mode = common.thermostatSystemModesLegacy[1];
                    break;
                }
            }
        ...
        convert: (model, msg, publish, options, meta) => {
            ...
            if (typeof msg.data[0x4008] == 'number') {
                ...
                if ((result.eurotronic_host_flags & 1 << 2) != 0) {
                    ...
                    result.system_mode = common.thermostatSystemModesLegacy[4];
                    resultHostFlags.boost = true;
                } else if ((result.eurotronic_host_flags & (1 << 4)) != 0 ) {
                    // system_mode => 'off', window open detected
                    result.system_mode = common.thermostatSystemModesLegacy[0];
