    ...
    plugwise_radiator_valve: {
        ...
        convert: (model, msg, publish, options, meta) => {
            if (!utils.isLegacyEnabled(options)) {
                return plugwise_thermostat.convert(model, msg, publish, options, meta);
            }

            const result = legacy.thermostat_att_report.convert(model, msg, publish, options, meta);
            if (typeof msg.data[0x4003] == 'number') {
                result.current_heating_setpoint =
            ...
            if (typeof msg.data[0x4002] == 'number') {
                result.plugwise_error_status = msg.data[0x4002];
            }
            if (typeof msg.data[0x4001] == 'number') {
                result.plugwise_valve_position = msg.data[0x4001];
            }
            return result;
        },
    },
