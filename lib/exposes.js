...
class Switch extends Base {
    ...
    withState(property, toggle) {
        ...
        const feature = new Binary('state', 'rw', 'ON', 'OFF').withProperty(property);
        if (toggle) {
...
class Climate extends Base {
    ...
    withFanMode(modes) {
        ...
        modes.forEach((m) => assert(allowed.includes(m)));
        this.features.push(new Enum('fan_mode', 'rw', modes));
        return this;
    }

    withPreset(modes) {
        assert(!this.endpoint, 'Cannot add feature after adding endpoint');
        this.features.push(new Enum('preset', 'rw', modes));
        return this;
    }

    withAwayMode() {
        assert(!this.endpoint, 'Cannot add feature after adding endpoint');
        this.features.push(new Binary('away_mode', 'rw', 'ON', 'OFF'));
        return this;
    }
}
