...
class Light extends Base {
    ...
    withPreferredColorMode(type) {
        assert(['hs', 'xy'].includes(type), `Preferred color mode ${type} is not allowed`);
        assert(this.features.find((e) => e.name === `color_${type}`), `Cannot make ${type} preferred since light doesnt support it`);
        this.preferredColorMode = type;
    }
