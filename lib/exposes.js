...
class Climate extends Base {
    ...
    withSetpointLimit(property, min, max, step, access=a.ALL) {
        assert(!this.endpoint, 'Cannot add feature after adding endpoint');
        assert(['min_heat_setpoint_limit', 'max_heat_setpoint_limit', 'min_cool_setpoint_limit', 'max_cool_setpoint_limit'].includes(property));
        this.features.push(new Numeric(property, access)
            .withValueMin(min).withValueMax(max).withValueStep(step).withUnit('Â°C').withDescription('Temperature setpoint limit'));
        return this;
    }
