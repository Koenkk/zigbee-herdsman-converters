...
module.exports = {
    ...
    options: {
        // TODO: also allows `{30: 0, 10: 1}` which is not supported
        calibration: (name, type='absolute') => new Numeric(`${name}_calibration`).withDescription(`Calibrates the ${name} value (${type} offset), takes into effect on next report of device.`),
        precision: (name) => new Numeric(`${name}_precision`).withValueMin(0).withValueMax(3).withDescription(`Number of digits after decimal point for ${name}, takes into effect on next report of device.`),
        ...
        color_sync: () => new Binary(`color_sync`, null, true, false).withDescription(`When enabled colors will be synced, e.g. if the light supports both color x/y and color temperature a conversion from color x/y to color temperature will be done when setting the x/y color (default true).`),
        ...
        expose_pin: () => new Binary(`expose_pin`, null, true, false).withDescription(`Expose pin of this lock in the published payload (default false).`),
        occupancy_timeout: () => new Numeric(`occupancy_timeout`).withValueMin(0).withDescription('Time in seconds after which occupancy is cleared after detecting it (default 90 seconds).'),
        vibration_timeout: () => new Numeric(`vibration_timeout`).withValueMin(0).withDescription('Time in seconds after which vibration is cleared after detecting it (default 90 seconds).'),
        // TODO: can also be an object (with delta/interval) which is not supported
        simulated_brightness: () => new Binary(`simulated_brightness`, null, true, false).withDescription(`TODO`),
        // TODO: we cannot express an array, example value: [10, 60]
        no_occupancy_since: () => new Binary(`no_occupancy_since`, null, true, false),
        precense_timeout: () => new Numeric(`precense_timeout`).withValueMin(0).withDescription('Time in seconds after which presence is cleared after detecting it (default 100 seconds).'),
        no_position_support: () => new Binary('no_position_support', null, true, false).withDescription('Set to true when your device only reports position 0, 100 and 50 (in this case your device has an older firmware) (default false).'),
        transition: () => new Numeric(`transition`).withValueMin(0).withDescription('Controls the transition time (in seconds) of on/off, brightness, color temperature (if applicable) and color (if applicable) changes. Defaults to `0` (no transition).'),
        legacy: () => new Binary(`legacy`, null, true, false).withDescription(`Set to false to disable the legacy integration (highly recommended), will change structure of the published payload (default true).`),
        measurement_poll_interval: () => new Numeric(`measurement_poll_interval`).withValueMin(0).withDescription(`This device does not support reporting electric measurements so it is polled instead. The default poll interval is 60 seconds.`),
    },
    presets: {
