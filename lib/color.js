...
class ColorXY {
    ...
    toRGB() {
        ...
        // XXX: results in failed tests (bad tests?)
        ...
        if (isNaN(red) || Object.is(red, -0)) {
            red = 0;
        }

        if (isNaN(green) || Object.is(green, -0)) {
            green = 0;
        }

        if (isNaN(blue) || Object.is(blue, -0)) {
            blue = 0;
        }
...
function syncColorState(newState, oldState, options) {
    const colorTargets = [];
    const colorSync = (options && options.hasOwnProperty('color_sync')) ? (options.color_sync == true) : true;
    const result = {};
    ...
    if (newState.hasOwnProperty('color_mode')) {
        ...
        // fallback to xy unless color_temp found
        result.color_mode = newState.hasOwnProperty('color_temp') ? 'color_temp' : 'xy';
    }
    ...
    case 'xy':
        ...
        if (result.color.hasOwnProperty('x') && result.color.hasOwnProperty('y')) {
            ...
            if (colorTargets.includes('hs')) {
                Object.assign(result.color, xy.toHSV().rounded(4).toObject(false, false));
            }
        }
        break;
    case 'color_temp':
    ...
    // always prefer newState.color.*
    // e.g. when reading both xy and hs, we prefer the real values
    Object.assign(result.color, newState.color);
