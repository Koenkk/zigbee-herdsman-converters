...
async function getImageMeta(current, logger, device) {
    ...
    const image = images[device.modelID()]
        ...
        .sort((a, b) => {
            ...
            if (parseInt(a.version.replace(/\./g, ''), 10) < parseInt(b.version.replace(/\./g, ''), 10)) {
                return -1;
            }
            if (parseInt(a.version.replace(/\./g, ''), 10) > parseInt(b.version.replace(/\./g, ''), 10)) {
                return 1;
    ...
    return {
        fileVersion: parseInt(image.version, 10), // version in the firmare removes the zero padding.
        url: image.firmware,
    };
}
