...
async function getImageMeta(current, logger, device) {
    ...
    if (Object.keys(images).indexOf(device._modelID) === -1) {
        throw new Error(
            `The device '${device._modelID}' is not supported for OTA at this time.`,
        );
    }
    const useBetaChannel = true;
    const image = images[device._modelID]
    ...
    if (!image) {
        throw new Error(
            ...
            } channel for the device '${device._modelID}'`,
        );
    }

    return {
        fileVersion: parseInt(image.version),
        url: image.firmware,
    };
}
