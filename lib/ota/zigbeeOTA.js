...
async function getImageMeta(current, logger, device) {
    ...
    const manufacturerName = current.manufacturerName;
    ...
    // In the case of Tuya end Moes, additional identification is carried out through the manufacturerName.
    const image = images.find((i) => i.imageType === imageType && i.manufacturerCode === manufacturerCode &&
        (!i.modelId || i.modelId === modelId) && (!i.manufacturerName || i.manufacturerName === manufacturerName));
