...
const valueConverter = {
    ...
    schedule: (periodsNumber, timeConverter, tempConverter) => {
        return {
            ...
            to: (v) => {
                const schedule = v.split(' ');
                const payload = [];

                for (let i = 0; i < periodsNumber; i++) {
                    const [timeRaw, tempRaw] = schedule[i].split('/');
                    const [hoursRaw, minutesRaw] = timeRaw.split(':', 2);
                    const [hours, minutes] = timeConverter(i).to([parseInt(hoursRaw), parseInt(minutesRaw)]);
                    const temp = tempConverter(i).to(parseFloat(tempRaw));
                    payload.push(...hours, ...minutes, ...temp);
                }
