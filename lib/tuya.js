...
const coverPositionInvert = [
    ...
    '_TZE200_wmcdj3aq', // Copied from previous TS0601_roller_blind
    '_TZE200_cowvfni3', // Copied from previous TS0601_roller_blind
    '_TZE200_rddyvrci', // Copied from previous TS0601_roller_blind
];
const coverStateOverride = {
    // Contains all covers which differentiate from the default enum states
    // Array values: [close, open, stop]
    // Use manufacturerName to identify device!
    '_TZE200_5zbp6j0u': [2, 0, 1], // Copied from previous TS0601_curtain
    '_TZE200_nkoabg8w': [2, 0, 1], // Copied from previous TS0601_curtain
    '_TZE200_xuzcvlku': [2, 0, 1], // Copied from previous TS0601_curtain
    '_TZE200_4vobcgd3': [2, 0, 1], // Copied from previous TS0601_curtain
    '_TZE200_nogaemzt': [2, 0, 1], // Copied from previous TS0601_curtain
    '_TZE200_pk0sfzvr': [2, 0, 1], // Copied from previous TS0601_curtain
    '_TZE200_fdtjuw7u': [2, 0, 1], // Copied from previous TS0601_curtain
    '_TZE200_zpzndjez': [2, 0, 1], // Copied from previous TS0601_curtain
    '_TZE200_rddyvrci': [1, 0, 2], // https://github.com/Koenkk/zigbee2mqtt/issues/5596#issuecomment-759408189
};

// Gets a boolean indicating whether the cover by this identifier (manufacturerName) needs reversed positions
function isCoverInverted(identifier) {

    if(coverPositionInvert.indexOf(identifier) !== -1)
        return true; // Return true if cover is listed in coverPositionInvert

    return false; // Return false by default, not inverted
}

// Gets an array containing which enums have to be used in order for the correct close/open/stop commands to be sent
function getCoverStateEnums(identifier) {
    const stateOverride = coverStateOverride[identifier];

    if(stateOverride !== undefined)
        return stateOverride; // Return the state override if it exists

    return [0, 2, 1]; // Return default enums
}
