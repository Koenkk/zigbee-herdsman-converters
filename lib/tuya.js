...
const dataPoints = {
    ...
    ZG_hum_value: 2,
    ZG_Battery: 4,
    ZG_temperature_unit: 9,
    ZG_temp_calibration: 23,
    ZG_hum_calibration: 24,
    ZG_illuminance: 101,
    ZG_door_state: 1,
    ZG_door_battery: 2,
};
...
const tuyaExposes = {
    ...
    contact: () => exposes.binary('contact', ea.STATE, false, true).withDescription('Indicates if the contact is closed (= true) or open (= false)'),
    illuminance: () => exposes.numeric('illuminance', ea.STATE).withUnit('lx').withDescription('Raw measured illuminance'),
    battery: () => exposes.numeric('battery', ea.STATE).withUnit('%').withDescription('Remaining battery in %').withValueMin(0).withValueMax(100),
    temperature: () => exposes.numeric('temperature', ea.STATE).withUnit('¡ãC').withDescription('Measured temperature value'),
    humidity: () => exposes.numeric('humidity', ea.STATE).withUnit('%').withDescription('Measured relative humidity'),
    zg_temperature_unit: () => exposes.enum('zg_temperature_unit', ea.STATE_SET, ['Celsius', 'Fahrenheit']).withDescription('Temperature unit'),
    zg_temp_calibration: () => exposes.numeric('zg_temp_calibration', ea.STATE_SET).withValueMin(-2.0).withValueMax(2.0).withValueStep(0.1).withUnit('¡ãC').withDescription('Temperature calibration'),
    zg_hum_calibration: () => exposes.numeric('zg_hum_calibration', ea.STATE_SET).withValueMin(-30).withValueMax(30).withValueStep(1).withUnit('%').withDescription('Humidity calibration'),
...
const valueConverter = {
    ...
    inverse: valueConverterBasic.inverse(),
    phaseA: {
        from: (v) => {
    ...
    ZGtemperature_unit: valueConverterBasic.lookup({ 'Celsius': 0, 'Fahrenheit': 1 }),
...
const tzDataPoints = {
    key: [
        ...
        'zg_temperature_unit', 'zg_temp_calibration', 'zg_hum_calibration',
