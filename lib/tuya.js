...
const valueConverter = {
    ...
    heatIdle: valueConverterBasic.lookup({'heat': true, 'idle': false}),
    ...
    schedule: {
        from: (v) => {
            ...
            for (let i = 0; i < periodsNumber; i++) {
                const hours = v[i * periodSize];
                const minutes = v[i * periodSize + 1];
                const tempHexArray = [v[i * periodSize + 2], v[i * periodSize + 3]];
                const tempRaw = Buffer.from(tempHexArray).readUIntBE(0, tempHexArray.length);
...
const tzDataPoints = {
    key: [
        ...
        'deadzone_temperature', 'upper_temp',
    ],
