...
const tuyaExposes = {
    ...
    backlightMode: () => exposes.enum('backlight_mode', ea.ALL, ['low', 'medium', 'high']).withDescription('Intensity of the backlight'),
...
const valueConverter = {
    ...
    idleHeat: tuya.valueConverterBasic.lookup({'idle': 1, 'heat': 0}),
    autoHeatOff: tuya.valueConverterBasic.lookup({'auto': 0, 'heat': 1, 'off': 2}),
...
const tuyaTz = {
    ...
    backlight_indicator_mode: {
        ...
        convertSet: async (entity, key, value, meta) => {
            const lookup = key === 'backlight_mode' ? {'low': 0, 'medium': 1, 'high': 2} : {'off': 0, 'off/on': 1, 'on/off': 2, 'on': 3};
            value = value.toLowerCase();
            utils.validateValue(value, Object.keys(lookup));
    ...
    datapoints: {
        key: [
            ...
            'schedule_wednesday', 'schedule_thursday', 'schedule_friday', 'schedule_saturday', 'schedule_sunday', 'clear_fault', 'scale_protection',
            'error',
        ],
        convertSet: async (entity, key, value, meta) => {
...
const tuyaExtend = {
    switch: (options={}) => {
        ...
        if (options.backlightMode || options.indicatorMode) {
            fromZigbee.push(options.backlightMode ? tuyaFz.backlight_mode : tuyaFz.indicator_mode);
