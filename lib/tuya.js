...
const tuyaTz = {
    ...
    datapoints: {
        key: [
            ...
            'reverse_direction', 'border', 'click_control', 'backlight_mode', 'accurate_calibration', 'backlight_switch', 'motor_steering', 
        ...
        convertSet: async (entity, key, value, meta) => {
            ...
            const tzHandlerCountForKey = meta.mapped.toZigbee.filter(i => i.key.includes(key)).length;
            const throwErrorOnDPUndefined = tzHandlerCountForKey === 1;
            
            for (const [key, value] of Object.entries(meta.message)) {
                const convertedKey = utils.postfixWithEndpointNameToZigbee(key, meta);
                const dpEntry = datapoints.find((d) => d[1] === convertedKey);
                if (!dpEntry || !dpEntry[1]) {
                    // Cannot handle datapoint but has another handler for that key
                    if (throwErrorOnDPUndefined) {
                        throw new Error(`No datapoint defined for '${key}'`);
                    } else {
                        meta.logger.debug(`No datapoint defined for '${key}'`); 
                        continue;                   
                    }   
                }
