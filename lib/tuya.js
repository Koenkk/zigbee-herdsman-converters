...
const valueConverter = {
    ...
    lightTypeToEnum: valueConverterBasic.lookup({'led': new Enum(0), 'incandescent': new Enum(1), 'halogen': new Enum(2)}),
    ...
    brightness: (stateDataPoint) => {
        return {
            from: valueConverter.scale0_254to0_1000.from,
            to: async (v, meta) => {
                // When brightness 0 is set, the light does not turn off, force it here.
                if (v === 0) {
                    const entity = meta.device.endpoints[0];
                    await sendDataPointBool(entity, stateDataPoint, false, 'dataRequest', 1);
                }
                return valueConverter.scale0_254to0_1000.to(v, meta);
            },
        };
    },
    phaseVariant1: {
