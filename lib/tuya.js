...
const tuyaExtend = {
    switch: (options={}) => {
        const exposes = options.endpoints ? options.endpoints.map((endpoint) => e.switch().withEndpoint(endpoint)) : [e.switch()];
        const fromZigbee = [fz.on_off, fz.ignore_basic_report];
        const toZigbee = [tz.on_off];
        if (options.powerOnBehavior) {
        ...
        if (options.backlightMode) {
            fromZigbee.push(tuyaFz.backlight_mode);
            toZigbee.push(tuyaTz.backlight_mode);
            exposes.push(tuyaExposes.backlightMode());
        }
