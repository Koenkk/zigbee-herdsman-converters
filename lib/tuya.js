...
function convertRawToCycleTimer(value) {
    ...
    if (value.length > 11) {
      ...
      var minsincemidnight = value[4] * 256 + value[5];
      ...
      irrigation_duration = value[8] * 256 + value[9];
      pause_duration = value[10] * 256 + value[11];
    }
    return { timernr: timernr, starttime: starttime, endtime: endtime, irrigation_duration: irrigation_duration, pause_duration: pause_duration, weekdays: weekdays, active: timeractive };
}
...
function convertRawToTimer(value) {
    ...
    if (value.length > 12) {
      timernr = value[1];
      minsincemidnight = value[2] * 256 + value[3];
      starttime = String(parseInt(minsincemidnight / 60)).padStart(2, "0") + ":" + String(minsincemidnight % 60).padStart(2, "0");
...
function convertTimeTo2ByteHexArray(time) {
    ...
    if (time_array.length != 2) {
      throw Exception("Time format incorrect");
    }
    var time_h = parseInt(time_array[0]);
    var time_m = parseInt(time_array[1]);
  
    if (time_h > 23 || time_m > 59) {
      throw Exception("Time incorrect");
    }
    return tuya.convertDecimalValueTo2ByteHexArray(time_h * 60 + time_m);
}
