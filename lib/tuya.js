...
const tuyaTz = {
    ...
    datapoints: {
        ...
        convertSet: async (entity, key, value, meta) => {
            ...
            const ignoreUndefinedDatapoints = meta.mapped.meta.ignoreUndefinedDatapoints || false;
            const dpWithoutEndpoints = meta.mapped.meta.dpWithoutEndpoints || false;
            const multiEndpoint = meta.mapped.meta.multiEndpoint || false;
            
            const useEndpointPrefix = multiEndpoint && !dpWithoutEndpoints;
            
            for (const [key, value] of Object.entries(meta.message)) {
                const convertedKey = useEndpointPrefix ? `${key}_${meta.endpoint_name}` : key;
