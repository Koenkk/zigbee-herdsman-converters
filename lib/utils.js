...
function postfixWithEndpointName(value, msg, definition, meta) {
    ...
    // multiEndpointSkip can be an array or a hash, ensure we always have an array below
    let multiEndpointSkip = definition.meta.multiEndpointSkip;
    if ((typeof multiEndpointSkip == 'object') && !Array.isArray(multiEndpointSkip)) {
        multiEndpointSkip = Object.keys(multiEndpointSkip);
    }

    if (definition.meta && definition.meta.multiEndpoint &&
            (!multiEndpointSkip || !multiEndpointSkip.includes(value))) {
...
function getMultiEndpoint(entity, key, meta) {
    const multiEndpointSkip = getMetaValue(entity, meta.mapped, 'multiEndpointSkip', 'allEqual', []);
    if (multiEndpointSkip && multiEndpointSkip.hasOwnProperty(key)) {
        const endpoint = entity.getDevice().getEndpoint(multiEndpointSkip[key]);
