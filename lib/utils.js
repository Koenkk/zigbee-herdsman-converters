...
function getSimulatedBrightnessKeys(options, msg, definition) {
    const keyPrefix = 'simulated_brightness_';
    let brightnessKey = keyPrefix + 'brightness';
    let directionKey = keyPrefix + 'direction';
    let timerKey = keyPrefix + 'timer';

    if (options.simulated_brightness &&
        options.simulated_brightness.multi_endpoint) {
        const property = postfixWithEndpointName('brightness', msg, definition);
        brightnessKey += `_${property}`;
        directionKey += `_${property}`;
        timerKey += `_${property}`;
    }

    return {brightness: brightnessKey, direction: directionKey, timer: timerKey};
}
