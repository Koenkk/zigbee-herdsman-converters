...
function batteryVoltageToPercentage(voltage, option) {
    ...
    // Generic converter that expects an option value to be given in the format of:
    // GENERIC_%MinimumThresholdVoltage%_%MaximumThresholdVoltage%
    // I.E. GENERIC_1900_3000
    } else if (optionSplit[0] === 'GENERIC') {
        if (optionSplit[1] && optionSplit[2]) {
            const thresholdMin = parseInt(optionSplit[1]);
            const thresholdMax = parseInt(optionSplit[2]);
            if ((thresholdMin && thresholdMax) && (thresholdMin < thresholdMax)) {
                percentage = toPercentage(voltage, thresholdMin, thresholdMax);
            } else {
                throw new Error(`batteryVoltageToPercentage could not parse option or minimum greater than maximum threshold: ${option}`);
            }
        } else {
            throw new Error(`batteryVoltageToPercentage generic converter used without minimum or maximum threshold: ${option}`);
        }
    } else {
