...
function batteryVoltageToPercentage(voltage, option) {
    ...
    // I.E. meta: {battery: {voltageToPercentage: {min: 1900, max: 3000}}}
        ...
        if (option.min && option.max) {
            if (option.min < option.max) {
                percentage = toPercentage(voltage, min, max);
            } else {
                throw new Error(`batteryVoltageToPercentage min (${option.min}) is greater than or equal to max (${option}.max)`);
            }
        } else {
            throw new Error(`batteryVoltageToPercentage missing min or max value`);
        }
    } else {
