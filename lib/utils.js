...
function batteryVoltageToPercentage(voltage, option) {
    ...
    // GENERIC_%Minimum threshold voltage%_%maximum threshold voltage% i.e. GENERIC_1900_3000
        ...
        if (optionSplit[1] && optionSplit[2]) {
            ...
            if ((thresholdMin && thresholdMax) && (thresholdMin < thresholdMax)) {
                ...
                throw new Error(`batteryVoltageToPercentage could not parse option or minimum threshold greater than maximum: ${option}`);
            }
        } else {
            throw new Error(`batteryVoltageToPercentage generic converter used without min or max threshold: ${option}`);
        }
    } else {
