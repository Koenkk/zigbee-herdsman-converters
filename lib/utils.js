...
function postfixWithEndpointNameToZigbee(key, meta) {
    const endpointName = meta.endpoint_name;
    const multiEndpoint = meta.mapped.meta.multiEndpoint || false;
    const multiEndpointSkip = meta.mapped.meta.multiEndpointSkip || [];

    if (multiEndpoint && (!multiEndpointSkip || !multiEndpointSkip.includes(key))) {
        return `${key}_${endpointName}`;
    }
...
module.exports = {
    ...
    postfixWithEndpointNameToZigbee,
    enforceEndpoint,
    getKey,
    getObjectProperty,
