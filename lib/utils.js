    ...
    if (option === '3V_2100') {
        ...
        if (optionSplit[1] && optionSplit[2]) {
            ...
            if ((thresholdMin && thresholdMax) && (thresholdMin < thresholdMax)) {
                ...
                throw new Error(`batteryVoltageToPercentage values could not be parsed or minimum threshold greater than maximum: ${option}`);
            }
        } else {
            throw new Error(`batteryVoltageToPercentage generic used converter used without min or max threshold: ${option}`);
        }
    } else {
