...
const PILOT_WIRE_CLUSTER = "customClusterVavPilotWire";
const manufacturerOptions = {manufacturerCode: 0x153a};

interface VavPilotWire {
    attributes: {
        mode: number;
    };
    commands: {
        ...
        toZigbee: [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    await entity.command<typeof PILOT_WIRE_CLUSTER, "setMode", VavPilotWire>(PILOT_WIRE_CLUSTER, "setMode", payload);
                    return {state: {pilot_wire_mode: value}};
                },
                convertGet: async (entity, key, meta) => {
                    await entity.read<typeof PILOT_WIRE_CLUSTER, VavPilotWire>(PILOT_WIRE_CLUSTER, [0x0000], manufacturerOptions);
                },
            },
        ],
        configure: [
            async (device, coordinatorEndpoint) => {
                ...
                if (configureReporting) {
                    const p = reporting.payload<typeof PILOT_WIRE_CLUSTER, VavPilotWire>("mode", 0, 120, 0, {min: 1, max: 3600, change: 0});
                    await ep.configureReporting(PILOT_WIRE_CLUSTER, p);
                } else {
                    await ep.read<typeof PILOT_WIRE_CLUSTER, VavPilotWire>(PILOT_WIRE_CLUSTER, ["mode"]);
                }
            },
        ],
        isModernExtend: true,
...
export const vavPilotWire = (configureReporting: boolean): ModernExtend[] => [pilotWireCluster, pilotWireConfig(configureReporting)];
