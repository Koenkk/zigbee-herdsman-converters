...
export class Light extends Base {
    ...
    withLevelConfig(disableFeatures: string[] = []) {
        ...
        if (!disableFeatures.includes('on_off_transition_time')) {
            levelConfig.withFeature(new Numeric('on_off_transition_time', access.ALL)
        ...
        if (!disableFeatures.includes('on_transition_time')) {
            levelConfig.withFeature(new Numeric('on_transition_time', access.ALL)
        ...
        if (!disableFeatures.includes('off_transition_time')) {
            levelConfig.withFeature(new Numeric('off_transition_time', access.ALL)
        ...
        if (!disableFeatures.includes('execute_if_off')) {
            levelConfig.withFeature(new Binary('execute_if_off', access.ALL, true, false)
        ...
        if (!disableFeatures.includes('on_level')) {
            levelConfig.withFeature(new Numeric('on_level', access.ALL)
        ...
        if (!disableFeatures.includes('current_level_startup')) {
            levelConfig.withFeature(new Numeric('current_level_startup', access.ALL)
                .withValueMin(1).withValueMax(254)
