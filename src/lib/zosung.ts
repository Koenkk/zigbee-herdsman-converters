...
export const fzZosung = {
    zosung_send_ir_code_01: {
        ...
        convert: (model, msg, publish, options, meta) => {
            meta.logger.debug(`"IR-Message-Code01" received (msg:${JSON.stringify(msg.data)})`, 'zhc:zosung');
            const seq = msg.data.seq;
            const irMsg = messagesGet(msg.endpoint, seq);
            meta.logger.debug(`IRCode to send: ${JSON.stringify(irMsg)} (seq:${seq})`, 'zhc:zosung');
        },
    ...
    zosung_send_ir_code_02: {
        ...
        convert: async (model, msg, publish, options, meta) => {
            meta.logger.debug(`"IR-Message-Code02" received (msg:${JSON.stringify(msg.data)})`, 'zhc:zosung');
            const seq = msg.data.seq;
            ...
            meta.logger.debug(`Sent IRCode part: ${part} (sum: ${sum}, seq:${seq})`, 'zhc:zosung');
        },
    } satisfies Fz.Converter,
    zosung_send_ir_code_04: {
        ...
        convert: async (model, msg, publish, options, meta) => {
            meta.logger.debug(`"IR-Message-Code04" received (msg:${JSON.stringify(msg.data)})`, 'zhc:zosung');
            const seq = msg.data.seq;
            ...
            meta.logger.debug(`IRCode has been successfully sent. (seq:${seq})`, 'zhc:zosung');
        },
    } satisfies Fz.Converter,
    zosung_send_ir_code_00: {
        ...
        convert: async (model, msg, publish, options, meta) => {
            meta.logger.debug(`"IR-Message-Code00" received (msg:${JSON.stringify(msg.data)})`, 'zhc:zosung');
            const seq = msg.data.seq;
            ...
            await msg.endpoint.command('zosungIRTransmit', 'zosungSendIRCode01',
                ...
                {disableDefaultResponse: true});
            meta.logger.debug(`"IR-Message-Code00" response sent.`, 'zhc:zosung');
            await msg.endpoint.command('zosungIRTransmit', 'zosungSendIRCode02',
                ...
                {disableDefaultResponse: true});
            meta.logger.debug(`"IR-Message-Code00" transfer started.`, 'zhc:zosung');
        },
    } satisfies Fz.Converter,
    zosung_send_ir_code_03: {
        ...
        convert: async (model, msg, publish, options, meta) => {
            meta.logger.debug(`"IR-Message-Code03" received (msg:${JSON.stringify(msg.data)})`, 'zhc:zosung');
            const seq = msg.data.seq;
            ...
            if (rcv.position==msg.data.position) {
                ...
                if (sum==expectedPartCrc) {
                    ...
                    meta.logger.debug(`${rcvMsgPart.length} bytes received.`, 'zhc:zosung');
                } else {
                    meta.logger.error(`Invalid msg part CRC: ${sum} expecting: ${expectedPartCrc}.`, 'zhc:zosung');
                }
            } else {
                meta.logger.error(`Unexpected IR code position: ${JSON.stringify(msg.data)}, expecting: ${rcv.position}.`, 'zhc:zosung');
            }
        },
    } satisfies Fz.Converter,
    zosung_send_ir_code_05: {
        ...
        convert: async (model, msg, publish, options, meta) => {
            meta.logger.debug(`"IR-Message-Code05" received (msg:${JSON.stringify(msg.data)})`, 'zhc:zosung');
            const seq = msg.data.seq;
            const rcv = messagesGet(msg.endpoint, seq);
            const learnedIRCode = rcv.buf.toString('base64');
            meta.logger.debug(`Received: ${learnedIRCode}`, 'zhc:zosung');
            messagesClear(msg.endpoint, seq);
...
export const tzZosung = {
    zosung_ir_code_to_send: {
        ...
        convertSet: async (entity, key, value, meta) => {
            if (!value) {
                meta.logger.error(`There is no IR code to send`, 'zhc:zosung');
                return;
            }
            ...
            meta.logger.debug(`Sending IR code: ${JSON.stringify(value)}`, 'zhc:zosung');
            ...
            await entity.command('zosungIRTransmit', 'zosungSendIRCode00',
                ...
                {disableDefaultResponse: true});
            meta.logger.debug(`Sending IR code initiated.`, 'zhc:zosung');
        },
    } satisfies Tz.Converter,
    zosung_learn_ir_code: {
        key: ['learn_ir_code'],
        convertSet: async (entity, key, value, meta) => {
            meta.logger.debug(`Starting IR Code Learning...`, 'zhc:zosung');
            await entity.command('zosungIRControl', 'zosungControlIRCommand00',
                {
                    data: Buffer.from(JSON.stringify({'study': 0})),
                },
                {disableDefaultResponse: true});
            meta.logger.debug(`IR Code Learning started.`, 'zhc:zosung');
        },
    } satisfies Tz.Converter,
};
