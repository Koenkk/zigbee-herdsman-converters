...
const SCENE_DATA = {
    'Ice land blue': [0x01, 0x15, 0x0a, 0x52, 0x52, 0xe0, 0x00, 0x00, 0x64, 0x00, 0xc1, 0x61, 0x00, 0xb4, 0x30, 0x00, 0xb5, 0x52, 0x00, 0xc4, 0x63],
    'Glacier express': [0x01, 0x16, 0x0a, 0x64, 0x64, 0x60, 0x00, 0x00, 0x64, 0x00, 0x92, 0x5f, 0x00, 0xc6, 0x60],
    'Sea of clouds': [0x01, 0x17, 0x03, 0x5e, 0x5e, 0x60, 0x00, 0x00, 0x64, 0x00, 0x38, 0x2f, 0x00, 0x1e, 0x5c, 0x00, 0xd5, 0x45, 0x01, 0x1a, 0x64],
    'Fireworks at sea': [0x01, 0x18, 0x02, 0x64, 0x64, 0xe0, 0x00, 0x00, 0x64, 0x00, 0xb2, 0x39, 0x01, 0x0a, 0x64, 0x01, 0x2d, 0x64, 0x01, 0x3f, 0x64],
    'Firefly night': [0x01, 0x1a, 0x03, 0x4b, 0x4b, 0xe0, 0x00, 0x00, 0x64, 0x00, 0xe0, 0x39, 0x01, 0x09, 0x53],
    'Grass land': [0x01, 0x1c, 0x0a, 0x5a, 0x5a, 0xe0, 0x00, 0x00, 0x52, 0x00, 0x9d, 0x64, 0x00, 0x8e, 0x64],
    'Northern lights': [0x01, 0x1d, 0x03, 0x52, 0x52, 0xe0, 0x00, 0x00, 0x64, 0x00, 0xae, 0x64, 0x00, 0xa6, 0x64, 0x00, 0xc1, 0x64, 0x00, 0xcc, 0x64],
    'Late autumn': [0x01, 0x1e, 0x0a, 0x52, 0x52, 0xe0, 0x00, 0x00, 0x64, 0x00, 0x19, 0x64, 0x00, 0x22, 0x5e, 0x00, 0x2c, 0x5b, 0x00, 0x14, 0x64, 0x00, 0x0c, 0x64],
    ...
    'Valentines Day': [0x01, 0x2a, 0x01, 0x64, 0x64, 0x60, 0x00, 0x00, 0x64, 0x01, 0x15, 0x64, 0x01, 0x05, 0x64, 0x01, 0x45, 0x64, 0x01, 0x2f, 0x64],
    'Neon world': [0x01, 0x37, 0x0a, 0x5a, 0x5a, 0x60, 0x00, 0x00, 0x64, 0x00, 0x33, 0x58, 0x00, 0x18, 0x64, 0x01, 0x00, 0x45, 0x00, 0xe3, 0x5e, 0x00, 0xac, 0x30]
} as const;
...
const tzTuyaSpiConverters = {
    tuya_spi_color_brightness: {
        ...
        convertSet: async (entity, key, value, meta) => {
            ...
            if (modeNow !== 'colour') {
                await tuya.sendDataPointEnum(ep, 2, 1); // 1=colour
            }
            
            if ('brightness' in meta.message) {
                const dp3 = mapBrightnessToDp3(meta.message.brightness);
    ...
    tuya_spi_scene: {
        ...
        convertSet: async (entity, key, value, meta) => {
            ...
            if (meta.state?.work_mode !== 'scene') {
                await tuya.sendDataPointEnum(ep, 2, 2); // 2=scene
            }
            
            const sceneName = value as keyof typeof SCENE_DATA;
    ...
    tuya_spi_music: {
        ...
        convertSet: async (entity, key, value, meta) => {
            ...
            if (meta.state?.work_mode !== 'music') {
                await tuya.sendDataPointEnum(ep, 2, 3); // 3=music
            }
            ...
            if (key === 'music_mode') {
                musicMode = value as keyof typeof MUSIC_DATA;
...
};
    {
        ...
        extend: [
            ...
            }),
            {
                ...
                toZigbee: [
                    tzTuyaSpiConverters.tuya_spi_color_brightness,
                    tzTuyaSpiConverters.tuya_spi_scene,
                    tzTuyaSpiConverters.tuya_spi_music,
                    tuya.tz.datapoints
                ],
                exposes: [
                    e.light_colorhs(),
                    e.numeric('brightness', exposes.access.STATE_SET).withValueMin(0).withValueMax(1000),
                    e.numeric('color_temp', exposes.access.STATE_SET).withValueMin(0).withValueMax(1000)
                        .withDescription('Color temperature (0=cold, 1000=warm)'),
                    e.enum('scene', exposes.access.STATE_SET, [
                        'Ice land blue', 'Glacier express', 'Sea of clouds', 'Fireworks at sea', 'Firefly night',
                        'Grass land', 'Northern lights', 'Late autumn', 'Game', 'Holiday', 'Party', 'Trend',
                        'Meditation', 'Dating', 'Valentines Day', 'Neon world'
                    ]).withDescription('Scenes selection'),
                    e.enum('music_mode', exposes.access.STATE_SET, [
                        'Rock', 'Jazz', 'Classic', 'Rolling', 'Energy', 'Spectrum'
                    ]).withDescription('Local music'),
                    e.numeric('music_sensitivity', exposes.access.STATE_SET).withValueMin(1).withValueMax(100)
                ...
                meta: {
                    tuyaDatapoints: [
                        ...
                        [2, 'work_mode', tuya.valueConverterBasic.lookup({
                            white: tuya.enum(0), colour: tuya.enum(1), scene: tuya.enum(2), music: tuya.enum(3),
                        })],
                        ...
                        [101, 'light_bead_sequence', tuya.valueConverterBasic.lookup({
                        ...
                        [102, 'chip_type', tuya.valueConverterBasic.lookup({
                            WS2801: tuya.enum(0), LPD6803: tuya.enum(1), LPD8803: tuya.enum(2), WS2811: tuya.enum(3),
                            TM1814B: tuya.enum(4), TM1934A: tuya.enum(5), SK6812: tuya.enum(6), SK9822: tuya.enum(7),
                            UCS8904B: tuya.enum(8), WS2805: tuya.enum(9),
                        })],
