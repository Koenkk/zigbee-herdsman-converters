...
const definition = {
	...
	exposes: [
		...
		e.enum('state', ea.STATE_SET, ['open', 'stop', 'close', 'lock', 'unlock'])
		...
		e.enum('mode', ea.STATE_SET, ['up', 'up_delete', 'remove_up_down'])
			.withDescription('Set mode'),
		// DP 5: motor runnning direction
		e.enum('control_back', ea.STATE_SET, ['forward', 'back'])
		...
		e.enum('work_state', ea.STATE, ['opening', 'closing', '123'])
			.withDescription('Work state'),
		// DP 10: Total Time (report only)
		e.numeric('time_total', ea.STATE).withValueMin(0).withValueMax(600).withUnit('s')
		...
		e.binary('fault', ea.STATE, true, false)
			.withDescription('Motor fault'),
		// DP 16: Border
		e.enum('border', ea.STATE_SET, ['down_delete', 'remove_top_bottom'])
			.withDescription('Set lower/upper limit'),
		// DP 19: Best position
		e.numeric('position_best', ea.STATE_SET).withValueMin(1).withValueMax(100) // error withValueMin(0)?
			.withDescription('Set the best position'),
		// DP 21: Angle control (numeric 0/25/50/75/100)
		e.numeric('angle_horizontal', ea.STATE_SET).withValueMin(0).withValueMax(100).withValueStep(25).withUnit('°')
			.withDescription('Set the angle'),
		// DP 101: Travel calibration
		e.enum('cur_calibration', ea.STATE_SET, ['start', 'end'])
			.withDescription('Calibrate the travel limits'),
		// DP 102: Quick calibration
		e.numeric('quick_calibration_1', ea.STATE_SET).withValueMin(0).withValueMax(900).withUnit('s')
			.withDescription('Set quick calibration'),
		// DP 103: Best position
		e.binary('switch', ea.STATE_SET, true, false)
			.withDescription('Trigger best position'),
		// DP 104: Reset
		e.enum('reset', ea.STATE_SET, ['reset'])
