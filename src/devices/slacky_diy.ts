...
import type {DefinitionWithExtend, ModernExtend, Expose, Fz, KeyValue, KeyValueAny, Tz} from "../lib/types";
import * as utils from "../lib/utils";
import {getOptions, getFromLookup, assertString, postfixWithEndpointName, precisionRound } from "../lib/utils"; 
...
const electricityMeterExtend = {
    ...
    const fromZigbee: Fz.Converter[] = [
      {
        ...
        convert: (model, msg, publish, options, meta) => {
          const result: KeyValueAny = {};
          if (msg.data.hasOwnProperty(attrElCityMeterModelName)) {
      ...
      },
      {
        ...
        convert: (model, msg, publish, options, meta) => {
          const result: KeyValueAny = {};
          if (msg.data.hasOwnProperty(attrElCityMeterDateRelease)) {
  ...
  deviceConfig: (): ModernExtend => {
    const exposes: Expose[] = [
      e.numeric('device_address_preset', ea.STATE_SET).withDescription('Device Address'),
    ...
    const fromZigbee: Fz.Converter[] = [
      {
        ...
        convert: (model, msg, publish, options, meta) => {
          const result: KeyValueAny = {};
          if (msg.data.hasOwnProperty(attrElCityMeterMeasurementPreset)) {
            const data = parseInt(msg.data[attrElCityMeterMeasurementPreset]);
...
function waterPreset(): ModernExtend {
  ...
  const exposes: Expose[] = [
    e.composite('preset', 'preset', ea.SET)
            .withFeature(e.numeric('hot_water_preset', ea.SET)
                          .withValueMin(0)
                          .withValueMax(99999999)
                          .withValueStep(1)
                          .withUnit('L')
                          .withDescription('Preset hot water'))
            .withFeature(e.numeric('cold_water_preset', ea.SET)
                          .withValueMin(0)
                          .withValueMax(99999999)
                          .withValueStep(1)
                          .withUnit('L')
                          .withDescription('Preset cold water'))
            .withFeature(e.numeric('step_water_preset', ea.SET)
                          .withValueMin(1)
                          .withValueMax(100)
                          .withValueStep(1)
                          .withUnit('L')
  ...
  const toZigbee: Tz.Converter[] = [{
      ...
      convertSet: async (entity, key, value, meta) => {
        ...
        if (values.hot_water !== undefined && values.hot_water >= 0) {
          const hot_water_preset = parseInt(values.hot_water);
          await endpoint.write('seMetering', {0xf000: {value: hot_water_preset, type: 0x23}})
        }
        if (values.cold_water !== undefined && values.cold_water >= 0) {
          const cold_water_preset = parseInt(values.cold_water);
          await endpoint.write('seMetering', {0xf001: {value: cold_water_preset, type: 0x23}})
        }
        if (values.step_water !== undefined && values.step_water >= 0) {
          const step_water_preset = parseInt(values.step_water);
          await endpoint.write('seMetering', {0xf002: {value: step_water_preset, type: 0x21}})
        }
      }
  },];
...
export const definitions: DefinitionWithExtend[] = [
    ...
    },
    {
      ...
      configure: async (device, coordinatorEndpoint, logger) => {
        ...
        const payload_date_release = [{attribute: {ID: attrElCityMeterDateRelease, type: 0x41}, minimumReportInterval: 0, maximumReportInterval: 300, reportableChange: 0}];
        await endpoint1.configureReporting('seMetering', payload_date_release);
        const payload_model_name = [{attribute: {ID: attrElCityMeterModelName, type: 0x41}, minimumReportInterval: 0, maximumReportInterval: 300, reportableChange: 0}];
