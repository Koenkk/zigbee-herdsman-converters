...
export const definitions: DefinitionWithExtend[] = [
    ...
    },
    {
        ...
        exposes: [
            e.numeric('total_forward_energy', ea.STATE).withUnit('kWh').withDescription('Total forward energy'),
            e.numeric('reverse_energy_total', ea.STATE).withUnit('kWh').withDescription('Reverse energy total'),
            e.numeric('phase_a', ea.STATE).withUnit('A').withDescription('Phase A current'),
            e.numeric('leakage_current', ea.STATE).withUnit('mA').withDescription('Leakage current'),
            e.numeric('temp_current', ea.STATE).withUnit('°C').withDescription('Temperature current'),
            ...
            e.numeric('timer', ea.ALL).withDescription('Timer (schedule control in seconds)'),
            ...
            e.text('alarm_set1', ea.ALL).withDescription('Alarm set 1 configuration'),
            e.text('alarm_set2', ea.ALL).withDescription('Alarm set 2 configuration'),
            e.text('alarm_set3', ea.ALL).withDescription('Alarm set 3 configuration'),
        ],

        meta: {
            tuyaDatapoints: [
                [1, 'total_forward_energy', tuya.valueConverter.divideBy100],
                [6, 'phase_a', tuya.valueConverter.divideBy1000], 
                [15, 'leakage_current', tuya.valueConverter.divideBy10], 
                [110, 'reverse_energy_total', tuya.valueConverter.divideBy100],
                [16, 'state', tuya.valueConverter.onOff],
                [9, 'fault', tuya.valueConverter.trueFalse],
                [103, 'temp_current', tuya.valueConverter.raw],
                [104, 'reclosing_enable', tuya.valueConverter.trueFalse],
                ...
                [11, 'switch_prepayment', tuya.valueConverter.trueFalse],
