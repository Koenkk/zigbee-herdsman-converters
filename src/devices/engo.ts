...
const definition = [
    {
        ...
        meta: {
            tuyaDatapoints: [
                ...
                [119, 'pump_delay_time', tuya.valueConverterBasic.lookup({
                    'OFF': tuya.enum(0),
                    '3 min': tuya.enum(1),
                    '5 min': tuya.enum(2),
                    '15 min': tuya.enum(3),
    ...
    },
    {
        ...
        options: [
            e.binary("expose_device_state", ea.SET, true, false)
                .withDescription("Expose device power state as a separate property when enabled."),
        ],
        exposes: (device, options) => {
            const exposesList = [
                ...
                e.enum('control_algorithm', ea.STATE_SET, ["TPI_UFH", "TPI_RAD", "TPI_ELE", "HIS_02", "HIS_04", "HIS_06", "HIS_08", "HIS_10", "HIS_20", "HIS_30", "HIS_40"]).withDescription("Control algorithm used to regulate temperature"),
                e.numeric("frost_set", ea.STATE_SET).withUnit("°C").withValueMin(5).withValueMax(17).withValueStep(0.5).withDescription("Frost protection setpoint"),
                e.numeric('holiday_temp_set', ea.STATE_SET).withUnit('°C').withValueMin(5).withValueMax(45).withValueStep(0.5).withDescription("Target temperature during holiday mode"),
                e.numeric('holiday_days_set', ea.STATE_SET).withUnit('day').withValueMin(1).withValueMax(30).withValueStep(1).withDescription("Number of days for holiday"),
                e.binary('valve_protection', ea.STATE_SET, 'ON', 'OFF').withDescription("Prevents valve blockage during long periods of inactivity."),
                e.enum("warm_floor", ea.STATE_SET, ["OFF", "7 min", "11 min", "15 min", "19 min", "23 min"]).withDescription("Automatically warms the floor every 60 minutes."),
            ...
            if (options?.expose_device_state === true) {
                exposesList.unshift(e.binary("state", ea.STATE_SET, "ON", "OFF")
            ...
            tuyaDatapoints: [
                [1, "state", {
                    ...
                    from: (value, meta, options) => {
                        ...
                        meta.state.system_mode = isOn
                            ? (meta.state.system_mode_device ?? "heat")
                            : "off";
                ...
                [3, "running_state", tuya.valueConverterBasic.lookup({
                    heat: tuya.enum(1),
                    cool: tuya.enum(3),
                }, "idle")],
                ...
                [43, "sensor_choose", tuya.valueConverterBasic.lookup({ 
                ...
                [58, "preset", tuya.valueConverterBasic.lookup({
                    manual: tuya.enum(0),
                ...
                [101, "control_algorithm", tuya.valueConverterBasic.lookup({ 
                    TPI_UFH: tuya.enum(0), TPI_RAD: tuya.enum(1), TPI_ELE: tuya.enum(2), HIS_02: tuya.enum(3),
                    HIS_04: tuya.enum(4), HIS_06: tuya.enum(5), HIS_08: tuya.enum(6), HIS_10: tuya.enum(7),
                    HIS_20: tuya.enum(8), HIS_30: tuya.enum(9), HIS_40: tuya.enum(10)
                })],
                ...
                [118, "warm_floor", tuya.valueConverterBasic.lookup({ 
                    OFF: tuya.enum(0), 
                    "7 min": tuya.enum(1), 
                    "11 min": tuya.enum(2), 
                    "15 min": tuya.enum(3), 
                ...
                [120, "sensor_error", tuya.valueConverterBasic.lookup({ 
                    Normal: tuya.enum(0), 
                    E1: tuya.enum(1), 
                    E2: tuya.enum(2) 
                })],
