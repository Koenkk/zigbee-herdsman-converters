...
const pctToByte = (v: number | string) => Math.max(0, Math.min(255, Math.round((Number(v) * 255) / 100)));
const byteToPct = (v: number | string) => Math.round((Number(v) * 100) / 255);
...
const tzGewiss: Tz.Converter = {
    ...
    convertSet: async (entity, key, value) => {
        ...
        const raw = pct ? pctToByte(value as number | string) : parseInt(String(value), 10);
        await entity.write(cluster, {[id]: {value: raw, type}}, opts);
        return {state: {[key]: pct ? Number(value) : parseInt(String(value), 10)}};
    },
...
const fzLed = {
    ...
    convert: (model: unknown, msg: Fz.Message<string>) => {
        const r: Record<string, unknown> = {};
        if (msg.data[0x0001] !== undefined) r.led_standby_brightness = byteToPct(msg.data[0x0001] as number | string);
        if (msg.data[0x0003] !== undefined) r.led_moving_brightness = byteToPct(msg.data[0x0003] as number | string);
        return r;
    },
};
