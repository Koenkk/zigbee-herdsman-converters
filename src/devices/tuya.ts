...
const definitions: Definition[] = [
    ...
    },
    {
        fingerprint: tuya.fingerprint('TS0601',
            ['_TZE200_yjjdcqsq', '_TZE200_9yapgbuv', '_TZE204_upagmta9', '_TZE200_cirvgep4', '_TZE200_upagmta9', '_TZE204_yjjdcqsq']),
    ...
    },
    {
        ...
        whiteLabel: [
            ...
            tuya.whitelabel('Rylike', 'RY-WS02Z', '2 gang socket outlet AU', ['_TZ3000_rgpqqmbj']),
        ],
        endpoint: (device) => {
            return {'l1': 1, 'l2': 2};
        },
    ...
    },
    {
        ...
        exposes: [
            ...
            e.numeric('entry_sensitivity', ea.STATE_SET).withValueMin(0).withValueMax(9).withValueStep(1)
                .withDescription('The sensitivity threshold triggered for the first time when the target enters the detection range. Mainly used to filter out accidental contact interference'),
            e.numeric('illuminance_threshold', ea.STATE_SET).withValueMin(0).withValueMax(420).withValueStep(0.1).withUnit('lx')
            ...
            e.numeric('entry_distance_indentation', ea.STATE_SET).withValueMin(0).withValueMax(8).withValueStep(0.1).withUnit('m')
                .withDescription('Indent the distance inward based on the detection distance. Mainly avoid false triggering caused by distance edge interference'),
            ...
            e.numeric('block_time', ea.STATE_SET).withValueMin(0).withValueMax(10).withValueStep(0.1).withUnit('sec')
                .withDescription('Set the block time to x seconds. After switching from occupied to unoccupied mode, it takes x seconds for the target to be detected again. Mainly solves the interference of sensors caused by power ripple and radiation generated by controlling external services'),
            e.binary('breaker_status', ea.STATE_SET, 'ON', 'OFF')
            ...
            e.enum('sensor', ea.STATE_SET, ['turn_on', 'turn_off', 'occupied', 'unoccupied'])
                .withDescription('The radar sensor can be set in four states: turn on, turn off, occupied, and unoccupied. For example, if occupied is set up, it will continue to maintain a presence regardless of whether someone present or not. If unoccupied is set, the unoccupied state will be maintained continuously.'),
        ],
        meta: {
            tuyaDatapoints: [
                [1, 'presence', tuya.valueConverter.trueFalse1],
