      ...
      priv = {
        ...
        recompute_power_ab: function (result: KeyValueAny) {
          ...
          if ("power_a" in result) {
            this.pub_power_a =
              result.power_a * (result.energy_flow_a === "producing" ? -1 : 1);
            modified = true;
          }
          if ("power_b" in result) {
            this.pub_power_b =
              result.power_b * (result.energy_flow_b === "producing" ? -1 : 1);
            modified = true;
          }
          if (modified) {
            if (this.pub_power_a !== null && this.pub_power_b !== null) {
              ...
              result.power_ab =
                Math.round(10 * this.pub_power_a + 10 * this.pub_power_b) / 10;
            }
          }
        },
        ...
        flush: function (
          ...
          this[`sign_${channel}`] =
            this[`power_${channel}`] =
            this[`current_${channel}`] =
            this[`power_factor_${channel}`] =
              null;
          ...
          if (
            ...
            const signedPower =
              options[signedPowerKey] != null ? options[signedPowerKey] : false;
            if (signedPower) {
              result[`power_${channel}`] = sign * power;
              result[`energy_flow_${channel}`] = "sign";
            } else {
              result[`power_${channel}`] = power;
              result[`energy_flow_${channel}`] =
                sign > 0 ? "consuming" : "producing";
            }
  ...
  energyFlowPJ1203A: (channel: string) => {
    return {
      from: (v: number, meta: Fz.Meta, options: KeyValue) => {
        ...
        const lateEnergyFlow =
          options[lateEnergyFlowKey] != null
            ? options[lateEnergyFlowKey]
            : false;
        if (lateEnergyFlow) {
...
const tzLocal = {
  ...
  led_control: {
    ...
    convertSet: async (entity, _key, _value, meta) => {
      ...
      let colorMode =
        meta.state.color_mode ?? colorModeLookup[ColorMode.ColorTemp];
      ...
      if ("color_temp" in meta.message)
        colorMode = colorModeLookup[ColorMode.ColorTemp];
      ...
      const transtime =
        typeof meta.message.transition === "number"
      ...
      if (colorMode === colorModeLookup[ColorMode.ColorTemp]) {
        ...
        if ("brightness" in meta.message || "color" in meta.message) {
          ...
          const brightness = utils.toNumber(
          ...
          const zclData = {
            ...
            hue: newSettings.hue,
            saturation: utils.mapNumberRange(
      ...
      Object.assign(
        newState,
        libColor.syncColorState(newState, meta.state, entity, meta.options),
      );
  ...
  TS0504B_color: {
    ...
    convertSet: async (entity, key, value, meta) => {
      ...
      const enableWhite =
        ...
        (color.isXY() &&
          (color.xy.x === 0.3125 || color.xy.y === 0.32894736842105265)) ||
      ...
      switch (key) {
        ...
        case "power_threshold": {
          ...
          const buf = Buffer.from([
            ...
            utils.getFromLookup(state, onOffLookup),
            0,
            utils.toNumber(value, "power_breaker"),
          ]);
          await entity.command("manuSpecificTuya_3", "setOptions2", {
            data: buf,
          });
        ...
        case "over_current_threshold": {
          ...
          const buf = Buffer.from([
            ...
            utils.getFromLookup(state, onOffLookup),
            0,
            utils.toNumber(value, "over_current_threshold"),
          ]);
        ...
        case "under_voltage_threshold": {
          ...
          const buf = Buffer.from([
            ...
            utils.getFromLookup(state, onOffLookup),
            0,
            utils.toNumber(value, "under_voltage_threshold"),
          ]);
          await entity.command("manuSpecificTuya_3", "setOptions3", {
            data: buf,
          });
  ...
  invert_cover_percent_fix: {
    ...
    convertSet: async (entity, key, value, meta) => {
      const shouldInvert =
        key === "position" && meta.options.cover_position_percent_fix;
      const newValue = shouldInvert ? 100 - Number(value) : value;
      return await legacy.toZigbee.tuya_cover_control.convertSet(
        entity,
        key,
        newValue,
        meta,
...
const fzLocal = {
  ...
  TS0222_humidity: {
    ...fz.humidity,
    convert: (model, msg, publish, options, meta) => {
      const result = fz.humidity.convert(
        model,
        msg,
        publish,
        options,
  ...
  scene_recall: {
    ...
    convert: (model, msg, publish, options, meta) => {
      ...
      const payload = {
        action: postfixWithEndpointName(
          `scene_${msg.data.sceneid}`,
          msg,
          model,
          meta,
        ),
  ...
  TS0201_humidity: {
    ...
    convert: (model, msg, publish, options, meta) => {
      if (
        ["_TZ3210_ncw88jfq", "_TZ3000_ywagc4rj"].includes(
  ...
  TS011F_electrical_measurement: {
    ...
    convert: async (model, msg, publish, options, meta) => {
      const result =
        (fz.electrical_measurement.convert(
          model,
          msg,
          publish,
          options,
      ...
      if (result) {
        for (const key of ["power", "current", "voltage"]) {
          if (key in result) {
            ...
            if (value === 0) {
              const configuredReporting =
                msg.endpoint.configuredReportings.find(
                  (c) =>
                    c.cluster.name === "haElectricalMeasurement" &&
                    c.attribute.name === lookup[key],
                );
              const time =
                (configuredReporting
                  ? configuredReporting.minimumReportInterval
                  : 5) *
                  2 +
                1;
  ...
  PJ1203A_strict_fz_datapoints: {
    ...
    convert: (model, msg, publish, options, meta) => {
      ...
      if (msg.data.seq !== expectedSeq && msg.data.seq !== priv.last_seq) {
        logger.debug(
          `[PJ1203A] Missing or re-ordered message detected: Got seq=${msg.data.seq}, expected ${priv.next_seq}`,
          NS,
        );
        priv.clear();
      }
...
export const definitions: DefinitionWithExtend[] = [
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel(
        "Tuya",
        "TS0205_smoke_2",
        "Optical smoke sensor (model YG500A on the PCB)",
        ["_TZ3210_up3pngle"],
      ),
      tuya.whitelabel("Nedis", "ZBDS10WT", "Smoke sensor", [
        "_TYZB01_wqcac7lo",
      ]),
    ],
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Niceboy", "ORBIS Windows & Door Sensor", "Door sensor", [
        "_TZ3000_qrldbmfn",
      ]),
      tuya.whitelabel("Tuya", "ZD06", "Door window sensor", [
        "_TZ3000_rcuyhwe3",
      ]),
      ...
      tuya.whitelabel("Moes", "ZSS-X-GWM-C", "Door/window magnetic sensor", [
        "_TZ3000_gntwytxo",
      ]),
      tuya.whitelabel("Luminea", "ZX-5232", "Smart door and window sensor", [
        "_TZ3000_4ugnzsli",
      ]),
      tuya.whitelabel("QA", "QASD1", "Door sensor", ["_TZ3000_udyjylt7"]),
      tuya.whitelabel("Nous", "E3", "Door sensor", ["_TZ3000_v7chgqso"]),
      tuya.whitelabel("Woox", "R7047", "Smart Door & Window Sensor", [
        "_TZ3000_timx9ivq",
      ]),
    ],
    ...
    meta: {
      tuyaDatapoints: [
        [
          101,
          "action",
          tuya.valueConverterBasic.lookup({ single: 0, double: 1, hold: 2 }),
        ],
      ],
    },
    whiteLabel: [
      tuya.whitelabel("Linkoze", "LKDSZ001", "Door sensor with scene switch", [
        "_TZ3210_jowhpxop",
      ]),
    ],
  },
  ...
  },
  {
    ...
    exposes: (device, options) => {
      ...
      if (
        ...
        exps.push(
          e
            .enum("battery_level", ea.STATE, ["low", "middle", "high"])
            .withDescription("Battery level state"),
        );
      }
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Tuya", "ZTH01", "Temperature and humidity sensor", [
        "_TZE200_yjjdcqsq",
        "_TZE204_yjjdcqsq",
        "_TZE284_yjjdcqsq",
      ]),
      tuya.whitelabel("Tuya", "SZTH02", "Temperature and humidity sensor", [
        "_TZE200_utkemkbs",
        "_TZE204_utkemkbs",
      ]),
      tuya.whitelabel("Tuya", "ZTH02", "Temperature and humidity sensor", [
        "_TZE200_9yapgbuv",
        "_TZE204_9yapgbuv",
      ]),
      tuya.whitelabel("Tuya", "ZTH05", "Temperature and humidity sensor", [
        "_TZE204_upagmta9",
        "_TZE200_upagmta9",
      ]),
      tuya.whitelabel("Tuya", "ZTH08-E", "Temperature and humidity sensor", [
        "_TZE200_cirvgep4",
        "_TZE204_cirvgep4",
      ]),
      tuya.whitelabel("Tuya", "ZTH08", "Temperature and humidity sensor", [
        "_TZE204_d7lpruvi",
        "_TZE284_d7lpruvi",
        "_TZE284_hdyjyqjm",
      ]),
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .numeric("max_humidity_alarm", ea.STATE_SET)
        .withUnit("%")
        .withValueMin(0)
        .withValueMax(100)
        .withDescription("Alarm humidity max"),
      e
        .numeric("min_humidity_alarm", ea.STATE_SET)
        .withUnit("%")
        .withValueMin(0)
        .withValueMax(100)
        .withDescription("Alarm humidity min"),
      e
        .enum("temperature_alarm", ea.STATE, [
          "lower_alarm",
          "upper_alarm",
          "cancel",
        ])
        .withDescription("Temperature alarm"),
      e
        .enum("humidity_alarm", ea.STATE, [
          "lower_alarm",
          "upper_alarm",
          "cancel",
        ])
        .withDescription("Humidity alarm"),
      e
    ...
    whiteLabel: [
      tuya.whitelabel(
        "ONENUO",
        "TH05Z",
        "Temperature & humidity sensor with clock and humidity display",
        ["_TZE200_vvmbj46n"],
      ),
    ],
  },
  ...
  },
  {
    ...
    fromZigbee: [
      legacy.fromZigbee.tuya_illuminance_temperature_humidity_sensor,
    ],
    toZigbee: [],
  ...
  },
  {
    ...
    exposes: [
      e
        .binary("carbon_monoxide", ea.STATE, "ON", "OFF")
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .binary("preheat", ea.STATE, true, false)
        .withDescription("Indicates sensor preheat is active"),
    ],
    whiteLabel: [
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Tuya", "QS-zigbee-S08-16A-RF", "Wall switch module", [
        "_TZ3000_dlhhrhs8",
      ]),
    ],
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("ClickSmart+", "CMA30036", "2 gang socket outlet", [
        "_TYZB01_hlla45kx",
      ]),
      ...
      tuya.whitelabel("Nova Digital", "NT-S2", "2 gang socket outlet BR", [
        "_TZ3000_sgb0xhwn",
      ]),
    ],
    endpoint: (device) => {
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Zemismart", "ZM-H7", "Hand wave wall smart switch", [
        "_TZ3000_jcqs2mrv",
      ]),
    ],
  },
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Lidl", "14156506L", "Livarno Lux smart LED mood light", [
        "_TZ3210_r0xgkft5",
      ]),
      tuya.whitelabel("Lidl", "HG08010", "Livarno Home outdoor spotlight", [
        "_TZ3210_umi6vbsz",
      ]),
      tuya.whitelabel("Lidl", "HG08008", "Livarno Home LED ceiling light", [
        "_TZ3210_p9ao60da",
      ]),
      tuya.whitelabel(
        "Lidl",
      ...
      tuya.whitelabel("Lidl", "14158804L", "Livarno Home LED desk lamp RGBW", [
        "_TZ3210_hxtfthp5",
      ]),
      tuya.whitelabel(
        "Lidl",
        "HG07834A/HG09155A/HG08131A",
        "Livarno Home GU10 spot RGB+CCT",
        ["_TZ3000_quqaeew6"],
      ),
      tuya.whitelabel(
        "Lidl",
        "HG07834B/HG09155B/HG08131B",
        "Livarno Home E14 candle RGB+CCT",
        ["_TZ3000_th6zqqy6"],
      ),
      tuya.whitelabel("Lidl", "HG07834B", "Livarno Home E14 candle RGB+CCT", [
        "_TZ3000_wr6g6olr",
      ]),
      tuya.whitelabel(
        "Lidl",
      ...
      tuya.whitelabel(
        "Lidl",
        "HG07834C/HG09155C/HG08131C",
        "Livarno Home E27 bulb RGB+CCT",
        ["_TZ3000_qd7hej8u"],
      ),
      tuya.whitelabel("Lidl", "HG08383B", "Livarno outdoor LED light chain", [
        "_TZ3000_bwlvyjwk",
      ]),
      tuya.whitelabel("Lidl", "HG08383A", "Livarno outdoor LED light chain", [
        "_TZ3000_taspddvq",
      ]),
      tuya.whitelabel(
        "Garza Smart",
      ...
      tuya.whitelabel(
        ...
        ["_TZ3210_dn5higyl", "_TZ3210_hicxa0rh"],
      ),
      tuya.whitelabel("Lidl", "HG08007", "Livarno Home outdoor LED band", [
        "_TZ3210_zbabx9wh",
      ]),
      tuya.whitelabel(
        "Lidl",
        "399629_2110",
        "Livarno Lux Ceiling Panel RGB+CCT",
        ["_TZ3210_c0s1xloa", "_TZ3210_x13bu7za"],
      ),
      tuya.whitelabel("Nous", "P3Z", "Smart light bulb", ["_TZ3210_cieijuw1"]),
      tuya.whitelabel("Moes", "ZLD-RCW_1", "RGB+CCT Zigbee LED controller", [
        "_TZ3000_7hcgjxpc",
      ]),
      tuya.whitelabel("Moes", "ZB-TD5-RCW-GU10", "RGB+CCT 4.7W GU10 LED bulb", [
        "_TZ3210_rcggc0ys",
      ]),
      tuya.whitelabel("Moes", "ZB-TDA9-RCW-E27-MS", "RGB+CCT 9W E27 LED bulb", [
        "_TZ3210_wxa85bwk",
      ]),
      ...
      tuya.whitelabel("Moes", "ZB-TDC6-RCW-E14", "RGB+CCT 5W E14 LED bulb", [
        "_TZ3210_ifga63rg",
      ]),
      tuya.whitelabel("MiBoxer", "E3-ZR", "3 in 1 LED Controller", [
        "_TZB210_wy1pyu1q",
      ]),
      tuya.whitelabel("MiBoxer", "FUT037Z+", "RGB led controller", [
        "_TZB210_417ikxay",
        "_TZB210_wxazcmsh",
      ]),
      tuya.whitelabel("MiBoxer", "FUT039Z", "RGB+CCT LED controller", [
        "_TZ3210_jicmoite",
        "_TZB210_rs0ufzwg",
      ]),
      tuya.whitelabel("MiBoxer", "FUT066Z", "RGB+CCT LED Downlight", [
        "_TZ3210_zrvxvydd",
      ]),
      tuya.whitelabel("MiBoxer", "FUT103ZR", "GU10 RGB+CCT", [
        "_TZB210_6eed09b9",
      ]),
      tuya.whitelabel("MiBoxer", "FUT106ZR", "GU10 RGB+CCT", [
        "_TZB210_rwy5hexp",
        "_TZB210_lnnkh3f9",
      ]),
      tuya.whitelabel(
        ...
        ["_TZ3210_jd3z4yig", "_TZ3210_r5afgmkl", "_TZ3210_mja6r5ix"],
      ),
      tuya.whitelabel("MiBoxer", "FUTC11ZR", "Outdoor light", [
        "_TZB210_zmppwawa",
      ]),
      tuya.whitelabel("TechToy", "_TZ3210_iw0zkcu8", "Smart bulb RGB 9W E27", [
        "_TZ3210_iw0zkcu8",
      ]),
    ],
    extend: [
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Tuya", "TS0505B_2_2", "Zigbee GU10/E14 5W smart bulb", [
        "_TZ3210_it1u8ahz",
      ]),
    ],
    ...
    exposes: [
      e
        .light_brightness_colortemp_colorhs([143, 500])
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("QA", "QADZC5", "5 in 1 LED controller", [
        "_TZB210_gj0ccsar",
      ]),
    ],
    extend: [
  ...
  },
  {
    ...
    exposes: [
      e
        .light_brightness_colortemp_colorhs([50, 1000])
        .removeFeature("color_temp_startup"),
      tuya.exposes.doNotDisturb(),
    ],
    configure: (device, coordinatorEndpoint) => {
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Lidl", "HG06463B", "Livarno Lux E27 G95 filament bulb", [
        "_TZ3000_nosnx7im",
      ]),
      tuya.whitelabel("Lidl", "HG06462A", "Livarno Lux E27 A60 filament bulb", [
        "_TZ3000_7dcddnye",
        "_TZ3000_nbnmw9nc",
      ]),
    ],
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Tuya", "L1(ZW)", "Light dimmer 0-10V", [
        "_TZB210_rkgngb5o",
      ]),
    ],
  },
  ...
  },
  {
    ...
    extend: [tuyaLight()],
    whiteLabel: [
      tuya.whitelabel("MiBoxer", "FUT036Z", "Single color LED controller", [
        "_TZ3210_dxroobu3",
        "_TZ3210_dbilpfqk",
      ]),
      tuya.whitelabel("Mercator Ikuü", "SMFL20W-ZB", "Ridley Floodlight", [
        "_TZ3000_juq7i1fr",
      ]),
    ],
  },
    ...
    meta: {
      tuyaDatapoints: [
        [
          102,
          "light",
          tuya.valueConverterBasic.lookup({ dark: false, bright: true }),
        ],
        [
          101,
          "action",
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Niceboy", "ORBIS Motion Sensor", "Motion sensor", [
        "_TZ3000_qomxlryd",
      ]),
      tuya.whitelabel("Luminea", "ZX-5311", "Motion sensor", [
        "_TZ3000_jmrgyl7o",
      ]),
      tuya.whitelabel("Tuya", "ZP01", "Motion sensor", ["_TZ3000_lf56vpxj"]),
      tuya.whitelabel("Tuya", "HW500A", "Motion sensor", ["_TZ3000_bsvqrxru"]),
      tuya.whitelabel("Nedis", "ZBSM10WT", "Motion sensor", [
        "_TZ3000_nss8amz9",
      ]),
    ],
    fromZigbee: [
    ...
    exposes: (device, options) => {
      ...
      if (
        !device ||
        !["_TZ3000_bsvqrxru", "_TZ3000_nss8amz9"].includes(
  ...
  },
  {
    ...
    fromZigbee: [
      ...
      fz.ignore_basic_report,
      fz.ZM35HQ_attr,
      legacy.fromZigbee.ZM35HQ_battery,
    ],
    ...
    configure: tuya.configureMagicPacket,
    whiteLabel: [
      tuya.whitelabel("Aubess", "40ZH-O", "Motion sensor", [
        "_TZ3040_msl6wxk9",
      ]),
    ],
  },
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("HOBEIAN", "ZG-222ZA", "Water leak sensor", [
        "_TZ3000_k4ej3ww2",
      ]),
      tuya.whitelabel(
        "Tuya",
        "TS0207_water_leak_detector_1",
        "Zigbee water flood sensor + 1m probe cable",
        ["_TZ3000_ocjlo4ea", "_TZ3000_upgcbody"],
      ),
      tuya.whitelabel(
        "Tuya",
        "TS0207_water_leak_detector_3",
        "Zigbee water leak sensor",
        ["_TYZB01_sqmd19i1"],
      ),
      tuya.whitelabel("Moes", "ZSS-QY-WL-C-MS", "Zigbee water leak sensor", [
        "_TZ3000_t6jriawg",
      ]),
      tuya.whitelabel("Tuya", "899WZ", "Water leak detector with 80DB Alarm", [
        "_TZ3000_mugyhz0q",
      ]),
      tuya.whitelabel("Niceboy", "ORBIS Water Sensor", "Water leak sensor", [
        "_TZ3000_awvmkayh",
      ]),
      tuya.whitelabel("Nous", "E4", "Water Leakage Sensor)", [
        "_TZ3000_0s9gukzt",
      ]),
    ],
    toZigbee: [],
  ...
  },
  {
    ...
    fromZigbee: [tuya.fz.datapoints],
    extend: [
      m.iasZoneAlarm({ zoneType: "rain", zoneAttributes: ["alarm_1"] }),
      m.battery({ percentageReporting: false }),
    ],
    exposes: [
      e.illuminance().withUnit("lx"),
      e
        .numeric("illuminance_average_20min", ea.STATE)
        .withUnit("lx")
        .withDescription("Illuminance average for the last 20 minutes"),
      e
        .numeric("illuminance_maximum_today", ea.STATE)
        .withUnit("lx")
        .withDescription("Illuminance maximum for the last 24 hours"),
      e
        .binary("cleaning_reminder", ea.STATE, true, false)
        .withDescription("Cleaning reminder"),
      e
        .numeric("rain_intensity", ea.STATE)
        .withUnit("mV")
        .withDescription("Rainfall intensity"),
    ],
    meta: {
      tuyaDatapoints: [
        [4, "battery", tuya.valueConverter.raw],
  ...
  {
    fingerprint: tuya.fingerprint("TS0101", [
      "_TYZB01_ijihzffk",
      "_TZ3210_tfxwxklq",
      "_TZ3210_2dfy6tol",
    ]),
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Mercator Ikuü", "SISW01", "Inline Switch", [
        "_TZ3210_2dfy6tol",
      ]),
    ],
    extend: [tuya.modernExtend.tuyaOnOff()],
  ...
  },
  {
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      await reporting.bind(device.getEndpoint(7), coordinatorEndpoint, [
        "genOnOff",
      ]);
    },
  },
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withValueMin(-10)
        .withValueMax(60)
        .withDescription("Soil temperature"),
      e
        .numeric("temperature_f", ea.STATE)
        .withUnit("°F")
        .withValueMin(14)
        .withValueMax(140)
        .withDescription("Soil temperature"),
      ...
      e
        ...
        .withValueMin(0)
        .withValueMax(100)
        .withDescription("Upper humidity limit"),
      e
        .numeric("min_humidity_alarm", ea.STATE_SET)
        .withUnit("%")
        .withValueMin(0)
        .withValueMax(100)
        .withDescription("Lower humidity limit"),
      e
        .numeric("schedule_periodic", ea.STATE_SET)
        .withUnit("min")
        .withValueMin(5)
        .withValueMax(60)
        .withDescription("Report sensitivity"),
      e.battery(),
      tuya.exposes.batteryState(),
    ],
    meta: {
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("GIEX", "GX04", "Soil Moisture Sensor", [
        "_TZE284_nhgdf6qr",
      ]),
    ],
  },
  {
    ...
    exposes: (device, options) => {
      ...
      if (
        !device ||
        !["_TZE200_ykgar0ow", "_TZE200_0hb4rdnp"].includes(
          device.manufacturerName,
        )
      ) {
        exps.push(
    ...
    meta: {
      tuyaDatapoints: [
        ...
        [
          21,
          "backlight_mode",
          tuya.valueConverter.backlightModeOffNormalInverted,
        ],
      ],
    },
    whiteLabel: [
      ...
      tuya.whitelabel(
        "Moes",
        "ZS-SR-EUD-1",
        "Star ring smart dimmer switch 1 gang",
        ["_TZE204_hlx9tnzb"],
      ),
      tuya.whitelabel("Moes", "MS-105Z", "Smart Dimmer module", [
        "_TZE200_la2c2uo9",
      ]),
      tuya.whitelabel("Mercator Ikuü", "SSWM-DIMZ", "Switch Mechanism", [
        "_TZE200_9cxuhakf",
      ]),
      tuya.whitelabel("Zemismart", "ZN2S-US1-SD", "Single gang dimmer", [
        "_TZE204_68utemio",
      ]),
      tuya.whitelabel("Mercator Ikuü", "SSWRM-ZB", "Rotary dimmer mechanism", [
        "_TZE200_a0syesf5",
      ]),
      tuya.whitelabel("Lonsonho", "EDM-1ZBB-EU", "Smart Dimmer Switch", [
        "_TZE200_0nauxa0p",
      ]),
      tuya.whitelabel("ION Industries", "ID200W-ZIGB", "LED Zigbee Dimmer", [
        "_TZE200_ykgar0ow",
      ]),
      tuya.whitelabel(
        "ION Industries",
        "90.500.090",
        "Zigbee Dimmer Master/Slave set",
        ["_TZE200_0hb4rdnp"],
      ),
    ],
  },
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel(
        "Moes",
        "ZS-SR-EUD-2",
        "Star ring smart dimmer switch 2 gangs",
        ["_TZE204_zenj4lxv"],
      ),
    ],
    extend: [
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel(
        "Moes",
        "ZS-SR-EUD-3",
        "Star ring smart dimmer switch 3 gangs",
        ["_TZE204_1v1dxkck"],
      ),
      tuya.whitelabel("Zemismart", "ZN2S-RS3E-DH", "3 gang dimmer", [
        "_TZE204_znvwzxkq",
        "_TZE284_znvwzxkq",
      ]),
    ],
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Moes", "ZM-105B-M", "2 gang smart dimmer module", [
        "_TZE204_bxoo2swd",
      ]),
      tuya.whitelabel("KnockautX", "FMD2C018", "2 gang smart dimmer module", [
        "_TZE200_tsxpl0d0",
      ]),
    ],
  },
    ...
    meta: {
      tuyaDatapoints: [
        ...
        [
          21,
          "indicator_mode",
          tuya.valueConverterBasic.lookup({ none: 0, relay: 1, pos: 2 }),
        ],
      ],
    },
    whiteLabel: [
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("ClickSmart+", "CMA30035", "1 gang socket outlet", [
        "_TYZB01_mtunwanm",
      ]),
  ...
  {
    fingerprint: tuya.fingerprint("TS110F", [
      "_TZ3000_estfrmup",
      "_TZ3000_ktuoyvt5",
    ]),
    model: "CSP051",
  ...
  {
    fingerprint: [
      ...tuya.fingerprint("isltm67\u0000", ["_TYST11_pisltm67"]),
      ...tuya.fingerprint("TS0601", ["_TZE200_pisltm67"]),
    ],
    model: "S-LUX-ZB",
    vendor: "Tuya",
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Danor", "SK-Z802C-US", "Smart curtain/shutter switch", [
        "_TZ3000_8h7wgocw",
      ]),
      { vendor: "LoraTap", model: "SC400" },
      tuya.whitelabel("LoraTap", "SC500ZB", "Roller Shutter Blind Module", [
        "_TZ3000_e3vhyirx",
      ]),
      tuya.whitelabel("Nous", "B4Z", "Curtain switch", ["_TZ3000_yruungrl"]),
      tuya.whitelabel("Nous", "L12Z", "Smart ZigBee Curtain Module L12Z", [
        "_TZ3000_jwv3cwak",
      ]),
      tuya.whitelabel("Zemismart", "ZN-LC1E", "Smart curtain/shutter switch", [
        "_TZ3000_74hsp7qy",
      ]),
    ],
    exposes: (device) => {
      ...
      if (device?.manufacturerName !== "_TZ3000_cet6ch1r") {
        ...
        exps.push(
          ...
          e
            .numeric("calibration_time_up", ea.STATE)
            .withUnit("s")
            .withDescription("Calibration time up"),
          e.binary("calibration_down", ea.ALL, "ON", "OFF"),
          e
            .numeric("calibration_time_down", ea.STATE)
            .withUnit("s")
            .withDescription("Calibration time down"),
        );
      }
      if (
        device?.manufacturerName !== "_TZ3210_xbpt8ewc" &&
        device?.manufacturerName !== "_TZ3000_e3vhyirx"
      ) {
        exps.push(
          tuya.exposes.indicatorMode(),
          tuya.exposes.backlightModeOffOn(),
        );
      }
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Girier", "TS130F_GIRIER", "Smart curtain switch", [
        "_TZ3210_dwytrmda",
      ]),
    ],
    ota: true,
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Yookee", "D10110_1", "Smart blind", [
        "_TZE200_9caxna4s",
      ]),
    ],
  },
  {
    ...
    whiteLabel: [
      ...
      { vendor: "Moes", model: "WS-EUB1-ZG" },
      { vendor: "AVATTO", model: "ZGB-WS-EU" },
      tuya.whitelabel("AVATTO", "WSMD-4", "4 gang switch", [
        "_TZE204_f5efvtbv",
        "_TZE284_f5efvtbv",
      ]),
      tuya.whitelabel("AVATTO", "ZWSMD-4", "4 gang switch", [
        "_TZE204_lbhh5o6z",
      ]),
    ],
    meta: {
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Mercator Ikuü", "SSW06G", "6 Gang switch", [
        "_TZE200_wnp4d4va",
      ]),
      tuya.whitelabel(
        "Nova Digital",
        "NTZB-04-W-B",
        "4 gang switch with 2 gang socket 4x4",
        ["_TZE204_lmgrbuwf"],
      ),
      tuya.whitelabel("Nova Digital", "SYZB-6W", "6 gang switch 4x4", [
        "_TZE284_tdhnhhiy",
      ]),
    ],
  },
  {
    ...
    toZigbee: [legacy.toZigbee.tuya_switch_state],
    whiteLabel: [
      tuya.whitelabel("Shawader", "SMKG-1KNL-US/TZB-W", "1 gang switch", [
        "_TZE204_ojtqawav",
      ]),
    ],
    configure: async (device, coordinatorEndpoint) => {
  ...
  },
  {
    ...
    toZigbee: [],
    whiteLabel: [
      tuya.whitelabel("Tuya", "BT400B", "Zigbee Panic Button", [
        "_TZ3000_0dumfk2z",
      ]),
      tuya.whitelabel("Woox", "R7052", "Smart SOS button", [
        "_TZ3000_ssp0maqm",
      ]),
    ],
    configure: async (device, coordinatorEndpoint) => {
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel(
        "Lidl",
        "HG06492C/HG08130C/HG09154C",
        "Livarno Lux E27 bulb CCT",
        ["_TZ3000_49qchf10"],
      ),
      tuya.whitelabel("Lidl", "14147206L", "Livarno Lux ceiling light", [
        "_TZ3000_rylaozuc",
        "_TZ3000_5fkufhn1",
      ]),
      tuya.whitelabel("Lidl", "14153905L", "Livarno Home LED floor lamp", [
        "_TZ3000_8uaoilu9",
      ]),
    ],
    configure: (device, coordinatorEndpoint) => {
  ...
  },
  {
    ...
    description: "Light controller",
    whiteLabel: [
      tuya.whitelabel("Mercator Ikuü", "SMI7040", "Ford Batten Light", [
        "_TZ3000_zw7wr5uo",
      ]),
      {
      ...
      tuya.whitelabel("MiBoxer", "E2-ZR", "2 in 1 LED controller", [
        "_TZB210_ayx58ft5",
        "_TZB210_eiwanbeb",
      ]),
      tuya.whitelabel("MiBoxer", "PZ2", "2 in 1 LED controller", [
        "_TZB210_0bkzabht",
      ]),
      tuya.whitelabel(
        "Lidl",
  ...
  },
  {
    ...
    exposes: [
      e
        .light_brightness_colortemp_colorhs([153, 500])
        .removeFeature("color_temp_startup"),
    ],
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Lidl", "HG06106B", "Livarno Lux E14 candle RGB", [
        "_TZ3000_odygigth",
      ]),
      tuya.whitelabel("Lidl", "HG06106A", "Livarno Lux GU10 spot RGB", [
        "_TZ3000_kdpxju99",
      ]),
      tuya.whitelabel("Lidl", "HG06106C", "Livarno Lux E27 bulb RGB", [
        "_TZ3000_dbou1ap4",
      ]),
      tuya.whitelabel("Lidl", "14148906L", "Livarno Lux mood light RGB+CCT", [
        "_TZ3000_9cpuaca6",
      ]),
      tuya.whitelabel(
        "Lidl",
        "14149505L/14149506L_1",
        "Livarno Lux light bar RGB+CCT (black/white)",
        ["_TZ3000_gek6snaj"],
      ),
      tuya.whitelabel("Mycket", "MS-SP-LE27WRGB", "E27 RGBW bulb", [
        "_TZ3000_evag0pvn",
      ]),
      tuya.whitelabel(
        "Lidl",
        "HG06104A",
        "Livarno Home RGB+CCT LED light strip 2m",
        ["_TZ3000_riwp3k79", "_TZ3000_riwp3k79"],
      ),
    ],
    configure: (device, coordinatorEndpoint) => {
      device.getEndpoint(1).saveClusterAttributeKeyValue("lightingColorCtrl", {
  ...
  },
  {
    ...
    fromZigbee: [
      fzLocal.TS0201_battery,
      fz.temperature,
      fzLocal.TS0201_humidity,
    ],
    toZigbee: [],
    ...
    whiteLabel: [
      ...
      tuya.whitelabel(
        "Tuya",
        "TS0201_1",
        "Zigbee 3.0 temperature humidity sensor with display",
        ["_TZ3210_alxkwn0h"],
      ),
      tuya.whitelabel(
        "Tuya",
      ...
      tuya.whitelabel("Tuya", "ZY-ZTH02", "Temperature and humidity sensor", [
        "_TZ3000_v1w2k9dd",
        "_TZ3000_rdhukkmi",
      ]),
      tuya.whitelabel("SEDEA", "eTH730", "Temperature and humidity sensor", [
        "_TZ3000_lqmvrwa2",
      ]),
      tuya.whitelabel("Moes", "ZSS-S01-TH", "Temperature and humidity sensor", [
        "_TZ3000_f2bw0b6k",
      ]),
      tuya.whitelabel(
        "Danfoss",
      ...
      tuya.whitelabel("Tuya", "HS09", "Hanging temperature humidity sensor", [
        "_TZ3000_1twfmkcc",
      ]),
      tuya.whitelabel("Nedis", "ZBSC10WT", "Temperature and humidity sensor", [
        "_TZ3000_fie1dpkm",
      ]),
    ],
  },
  {
    ...
    configure: tuya.configureMagicPacket,
    whiteLabel: [
      tuya.whitelabel("Tuya", "TH02Z", "Temperature and humidity sensor", [
        "_TZ3000_fllyghyj",
        "_TZ3000_saiqcn0y",
      ]),
      tuya.whitelabel(
  ...
  },
  {
    ...
    configure: tuya.configureMagicPacket,
    whiteLabel: [
      tuya.whitelabel("Tuya", "RSH-HS06_1", "Temperature & humidity sensor", [
        "_TZ3000_zl1kmjqx",
      ]),
    ],
  },
  {
    fingerprint: tuya.fingerprint("SM0201", [
      "_TYZB01_cbiezpds",
      "_TYZB01_zqvwka4k",
    ]),
    model: "SM0201",
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Tuya", "SH-SC07", "Button scene switch", [
        "_TZ3000_mrpevh8p",
      ]),
      tuya.whitelabel("Tuya", "MINI-ZSB", "Smart button", ["_TZ3000_qgwcxxws"]),
      tuya.whitelabel("Nous", "LZ4", "Wireless switch button", [
        "_TZ3000_6km7djcm",
      ]),
      tuya.whitelabel("Marmitek", "Push_LE", "Smart switch", [
        "_TZ3000_4upl1fcj",
      ]),
    ],
  ...
  },
  {
    ...
    description: "Wireless switch with 1 button",
    whiteLabel: [
      tuya.whitelabel("Cleverio", "SB100", "Wireless switch with 1 button", [
        "_TYZB01_4qw4rl1u",
      ]),
      tuya.whitelabel("Marmitek", "Push_ME", "Wireless switch with 1 button", [
        "_TYZB01_1xktopx6",
      ]),
    ],
    exposes: [
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Marmitek", "Push_LO", "Smart switch", [
        "_TZ3000_dfgbtub0",
      ]),
    ],
    exposes: [
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Moes", "ZT-SR-EU4", "Star Ring 4 Gang Scene Switch", [
        "_TZ3000_a4xycprs",
      ]),
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Zemismart", "ZMR4", "Wireless switch with 4 buttons", [
        "_TZ3000_11pg3ima",
        "_TZ3000_et7afzxz",
      ]),
    ],
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      await endpoint.read(
        "genBasic",
        [0x0004, 0x000, 0x0001, 0x0005, 0x0007, 0xfffe],
      );
      ...
      for (const ep of [1, 2, 3, 4]) {
        ...
        if (device.getEndpoint(ep)) {
          await reporting.bind(device.getEndpoint(ep), coordinatorEndpoint, [
            "genOnOff",
          ]);
        }
      }
  ...
  },
  {
    ...
    options: [
      e
        .enum("control_sequence_of_operation", ea.SET, [
          "cooling_only",
      ...
      e
        ...
        .withDescription(
          "Expose device power state as a separate property when enabled.",
        ),
    ],
    exposes: (device, options) => {
      ...
      const exposes = [
        ...
        e
          ...
          .withFeature(
            e
              .text("weekdays", ea.SET)
              .withDescription(
                'Schedule (1-5), 4 periods in format "hh:mm/tt".',
              ),
          )
          .withFeature(
            e
              .text("saturday", ea.SET)
              .withDescription('Schedule (6), 4 periods in format "hh:mm/tt".'),
          )
          .withFeature(
            e
              .text("sunday", ea.SET)
              .withDescription('Schedule (7), 4 periods in format "hh:mm/tt".'),
          )
          .withDescription(
      ...
      if (options?.expose_device_state === true) {
        exposes.unshift(
          e
            ...
            .withDescription("Turn the thermostat ON or OFF"),
        );
      }
    ...
    meta: {
      ...
      tuyaDatapoints: [
        [
          ...
          "state",
          {
            ...
            from: (v: boolean, meta: Fz.Meta, options?: KeyValue) => {
              meta.state.system_mode =
                v === true ? (meta.state.system_mode_device ?? "cool") : "off";
        ...
        [
          4,
          "preset",
          tuya.valueConverterBasic.lookup({ manual: true, auto: false }),
        ],
        ...
        [
          ...
          {
            to: (
              v: { weekdays: string; saturday: string; sunday: string },
              meta: Tz.Meta,
            ) => {
  ...
  },
  {
    ...
    fromZigbee: [
      fz.ignore_basic_report,
      fz.ignore_tuya_raw,
      legacy.fromZigbee.wls100z_water_leak,
    ],
    toZigbee: [],
    onEvent: tuya.onEventSetTime,
    configure: async (device, coordinatorEndpoint) => {
  ...
  },
  {
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      /*
            ...
            */
      if (
        "_TZ3000_xkap8wtb" === device.manufacturerName &&
        [162, 100].includes(device.applicationVersion)
      ) {
        logger.warning(
    ...
    whiteLabel: [
      tuya.whitelabel("Nous", "B2Z", "1 gang switch with power monitoring", [
        "_TZ3000_qlai3277",
      ]),
      tuya.whitelabel(
        "Colorock",
        "CR-MNZ1",
        "1 gang switch 30A with power monitoring",
        ["_TZ3000_tgddllx4"],
      ),
      tuya.whitelabel("Nous", "L6Z", "Switch with power monitoring", [
        "_TZ3000_qaabwu5c",
      ]),
    ...
    onEvent: async (type, data, device, options) => {
      if (["_TZ3000_x3ewpzyr"].includes(device.manufacturerName)) {
        await tuya.onEventMeasurementPoll(
          type,
          data,
          device,
          options,
        ...
        ["_TZ3000_xkap8wtb"].includes(device.manufacturerName) &&
        [162, 100].includes(device.applicationVersion)
      ) {
        await tuya.onEventMeasurementPoll(
          type,
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel(
        "Tuya",
        "XSH01B",
        "2 gang switch module with power monitoring",
        ["_TZ3000_irrmjcgi"],
      ),
      tuya.whitelabel(
        "Nous",
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Tuya", "XMSJ", "Zigbee USB power switch", [
        "_TZ3000_8n7lqbm0",
      ]),
    ],
    configure: async (device, coordinatorEndpoint) => {
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("PSMART", "T441", "1 gang switch module", [
        "_TZ3000_myaaknbq",
      ]),
      tuya.whitelabel("PSMART", "T461", "1 gang switch module", [
        "_TZ3000_cpozgbrx",
      ]),
    ],
    configure: async (device, coordinatorEndpoint) => {
...

  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Moes", "ZM-104B-M", "2 gang switch", [
        "_TZ3000_qaa59zqd",
      ]),
      tuya.whitelabel("pcblab.io", "RR620ZB", "2 gang Zigbee switch module", [
        "_TZ3000_4xfqlgqo",
      ]),
      tuya.whitelabel("Nous", "L13Z", "2 gang switch", [
        "_TZ3000_ruxexjfz",
        "_TZ3000_hojntt34",
      ]),
      ...
      tuya.whitelabel("Mercator Ikuü", "SSW02", "2 gang switch", [
        "_TZ3000_fbjdkph9",
      ]),
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("AVATTO", "ZWSM16-2-Zigbee", "2 gang switch module", [
        "_TZ3000_mtnpt6ws",
      ]),
      tuya.whitelabel("PSMART", "T442", "2 gang switch module", [
        "_TZ3000_mufwv0ry",
      ]),
      tuya.whitelabel("Lonsonho", "X702A", "2 gang switch with backlight", [
        "_TZ3000_54hjn4vs",
        "_TZ3000_aa5t61rh",
      ]),
      tuya.whitelabel(
      ...
      tuya.whitelabel("RoomsAI", "37022463-2", "2 Gang switch with backlight", [
        "_TZ3000_ogpla3lh",
      ]),
    ],
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel(
        "Zemismart",
        "TB26-2",
        "2 Gang switch with backlight, countdown, inching",
        ["_TZ3000_ywubfuvt"],
      ),
...

  {
    ...
    whiteLabel: [
      tuya.whitelabel("Lonsonho", "X703A", "3 Gang switch with backlight", [
        "_TZ3000_rhkfbfcv",
      ]),
      tuya.whitelabel("Zemismart", "ZM-L03E-Z", "3 gang switch with neutral", [
        "_TZ3000_empogkya",
        "_TZ3000_lsunm46z",
      ]),
      tuya.whitelabel("Tuya", "M10Z", "2 gang switch with 20A power socket", [
        "_TZ3000_lubfc1t5",
      ]),
    ],
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Tuya", "TS0003_1", "3 gang switch", [
        "_TZ3000_ouwfc1qj",
      ]),
      tuya.whitelabel("Zemismart", "TB26-3", "3 gang switch", [
        "_TZ3000_eqsair32",
      ]),
    ],
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("AVATTO", "ZWSM16-3-Zigbee", "3 gang switch module", [
        "_TZ3000_hbic3ka3",
      ]),
    ],
  },
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("AVATTO", "ZWSM16-1-Zigbee", "1 gang switch module", [
        "_TZ3000_4rbqgcuv",
      ]),
    ],
    extend: [
  ...
  {
    fingerprint: tuya.fingerprint("TS0004", [
      "_TZ3000_ltt60asa",
      "_TZ3000_mmkbptmx",
    ]),
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Yushun", "YS-MT750L", "Curtain motor", [
        "_TZE200_bqcqqjpb",
        "_TZE200_gaj531w3",
      ]),
      ...
      tuya.whitelabel(
        "Shenzhen Golden Security Technology",
        "GM46",
        "Curtain motor",
        ["_TZE204_guvc7pdy"],
      ),
      tuya.whitelabel("Roximo", "CRTZ01", "Curtain motor", [
        "_TZE204_57hjqelq",
      ]),
      { vendor: "Quoya", model: "AT8510-TY" },
      tuya.whitelabel("Somgoms", "ZSTY-SM-1DMZG-US-W_1", "Curtain switch", [
        "_TZE200_axgvo9jh",
      ]),
      tuya.whitelabel(
        "HUARUI",
        "CMD900LE",
        "Lithium battery intelligent curtain opening and closing motor",
        ["_TZE200_zxxfv8wi"],
      ),
      tuya.whitelabel("Novato", "WPK", "Smart curtain track", [
        "_TZE204_lh3arisb",
      ]),
      tuya.whitelabel("Zemismart", "ZMS1-TYZ", "Smart curtain track", [
        "_TZE204_zuq5xxib",
      ]),
    ],
    ...
    exposes: [
      ...
      e
        ...
        .withFeature(
          e
            .binary("reverse_direction", ea.STATE_SET, true, false)
  ...
  },
  {
    ...
    fromZigbee: [legacy.fromZigbee.tuya_cover, fz.ignore_basic_report],
    toZigbee: [
      tzLocal.invert_cover_percent_fix,
      legacy.toZigbee.tuya_cover_options,
    ],
    exposes: [
      e.cover_position().setAccess("position", ea.STATE_SET),
  ...
  {
    fingerprint: [
      ...tuya.fingerprint("TS0601", ["_TZE200_eegnwoyw"]),
      ...tuya.fingerprint("TS0105", ["_TZE600_ogyg1y6b"]),
    ],
    model: "TS0601_cover_2",
    vendor: "Tuya",
    description: "Curtain motor or roller blind motor with fixed speed",
    whiteLabel: [
      tuya.whitelabel("Zemismart", "BCM100DB", "Curtain Motor", [
        "_TZE200_eegnwoyw",
      ]),
      tuya.whitelabel("Nova Digital", "ZBCMR-01", "Roller Blind Motor", [
        "_TZE600_ogyg1y6b",
      ]),
    ],
    fromZigbee: [legacy.fromZigbee.tuya_cover, fz.ignore_basic_report],
  ...
  },
  {
    ...
    exposes: [
      ...
      e.battery(),
      e
        .enum("opening_mode", ea.STATE_SET, ["tilt", "lift"])
        .withDescription("Opening mode"),
      e
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withDescription("Reverse the motor direction"),
      e
        .binary("motor_fault", ea.STATE, true, false)
    ...
    whiteLabel: [
      tuya.whitelabel("Lilistore", "TS0601_lilistore", "Cover motor", [
        "_TZE204_r0jdjrvi",
      ]),
      tuya.whitelabel("Zemismart", "ZM90E-DT250N/A400", "Window opener", [
        "_TZE204_r0jdjrvi",
      ]),
      tuya.whitelabel("Nova Digital", "CMR-1", "Roller Blind Motor", [
        "_TZE200_9vpe3fl1",
      ]),
    ],
  },
    ...
    meta: {
      tuyaDatapoints: [
        ...
        [
          5,
          "motor_direction",
          tuya.valueConverterBasic.lookup({ normal: false, reversed: true }),
        ],
      ],
    },
    whiteLabel: [
      tuya.whitelabel("Nova Digital", "ZC-GM42", "Roller blind motor", [
        "_TZE204_ycke4deo",
      ]),
    ],
  },
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Immax", "07732L", "Radiator valve with thermostat", [
        "_TZE200_rufdtfyv",
      ]),
      { vendor: "Evolveo", model: "Heat M30" },
      tuya.whitelabel("Emos", "P5630S", "Radiator valve with thermostat", [
        "_TZE200_rk1wojce",
      ]),
    ],
    meta: {
    ...
    onEvent: tuya.onEventSetLocalTime,
    fromZigbee: [
      legacy.fromZigbee.tuya_thermostat,
      fz.ignore_basic_report,
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withUnit("min")
        .withDescription("Boost time remaining"),
      ...tuya.exposes
        .scheduleAllDays(ea.STATE_SET, "06:00/21.5 17:20/26 20:00/21 24:00/18")
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withValueMax(900),
      e
        .binary("window_open", ea.STATE, "OPEN", "CLOSE")
        .withDescription("Window status CLOSE or OPEN "),
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .enum("click_control", ea.STATE_SET, ["up", "down"])
        .withDescription("Single motor steps"),
      e.binary("motor_fault", ea.STATE, true, false),
    ],
    whiteLabel: [
      tuya.whitelabel("Zemismart", "ZM16EL-03/33", "Cover motor", [
        "_TZE200_68nvbio9",
      ]),
      tuya.whitelabel("Zemismart", "ZM25EL", "Cover motor", [
        "_TZE200_pw7mji0l",
      ]),
      tuya.whitelabel(
        "Zemismart",
        "ZM85EL-2Z",
        "Roman Rod I type U curtains track",
        ["_TZE200_cf1sl3tj", "_TZE200_nw1r9hp6"],
      ),
      tuya.whitelabel("Hiladuo", "B09M3R35GC", "Motorized roller shade", [
        "_TZE200_9p5xmj5r",
      ]),
    ],
    meta: {
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .enum("set_bottom_limit", ea.SET, ["SET"])
        .withDescription(
          "Set the bottom limit, to reset limits use factory_reset",
        ),
      e
        .binary("factory_reset", ea.SET, true, false)
        .withDescription("Factory reset the device"),
    ],
    whiteLabel: [
      tuya.whitelabel(
        "Moes",
        "AM43-0.45/40-ES-EB",
        "Roller blind/shades drive motor",
        ["_TZE200_zah67ekd", "_TZE200_icka1clh"],
      ),
    ],
    configure: (device, coordinatorEndpoint) => {
      device.powerSource = "Mains (single phase)";
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Moes", "ZTRV-ZX-TV01-MS", "Thermostat radiator valve", [
        "_TZE200_7yoranx2",
      ]),
      tuya.whitelabel("Moes", "TV01-ZB", "Thermostat radiator valve", [
        "_TZE200_e9ba97vf",
      ]),
      tuya.whitelabel("AlecoAir", "HA-08_THERMO", "Thermostat radiator valve", [
        "_TZE200_wsbfwodu",
      ]),
    ],
    ota: true,
    ...
    exposes: [
      ...
      e
        .binary("online", ea.STATE_SET, "ON", "OFF")
    ...
    meta: {
      tuyaDatapoints: [
        ...
        [
          46,
          "holiday_start_stop",
          tuya.valueConverter.thermostatHolidayStartStop,
        ],
        ...
        [
          108,
          "schedule_monday",
          tuya.valueConverter.thermostatScheduleDaySingleDP,
        ],
        [
          112,
          "schedule_tuesday",
    ...
    meta: {
      tuyaDatapoints: [
        ...
        [
          101,
          "schedule_monday",
          tuya.valueConverter.thermostatScheduleDayMultiDP,
        ],
        [
          102,
          "schedule_tuesday",
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("AVATTO", "ME168_1", "Thermostatic radiator valve", [
        "_TZE200_rxntag7i",
      ]),
      tuya.whitelabel("AVATTO", "TRV06_1", "Thermostatic radiator valve", [
        "_TZE200_hvaxb2tc",
        "_TZE284_o3x45p96",
      ]),
      tuya.whitelabel("EARU", "TRV06", "Smart thermostat module", [
        "_TZE200_yqgbrdyo",
        "_TZE200_rxq4iti9",
      ]),
      tuya.whitelabel("AVATTO", "AVATTO_TRV06", "Thermostatic radiator valve", [
        "_TZE284_c6wv4xyo",
        "_TZE204_o3x45p96",
      ]),
    ],
    ...
    meta: {
      tuyaDatapoints: [
        ...
        [
          28,
          "schedule_monday",
          tuya.valueConverter.thermostatScheduleDayMultiDPWithDayNumber(1, 6),
        ],
        [
          29,
          "schedule_tuesday",
          tuya.valueConverter.thermostatScheduleDayMultiDPWithDayNumber(2, 6),
        ],
        [
          30,
          "schedule_wednesday",
          tuya.valueConverter.thermostatScheduleDayMultiDPWithDayNumber(3, 6),
        ],
        [
          31,
          "schedule_thursday",
          tuya.valueConverter.thermostatScheduleDayMultiDPWithDayNumber(4, 6),
        ],
        [
          32,
          "schedule_friday",
          tuya.valueConverter.thermostatScheduleDayMultiDPWithDayNumber(5, 6),
        ],
        [
          33,
          "schedule_saturday",
          tuya.valueConverter.thermostatScheduleDayMultiDPWithDayNumber(6, 6),
        ],
        [
          34,
          "schedule_sunday",
          tuya.valueConverter.thermostatScheduleDayMultiDPWithDayNumber(7, 6),
        ],
        ...
        [
          47,
          "local_temperature_calibration",
          tuya.valueConverter.localTempCalibration2,
        ],
  ...
  {
    fingerprint: [
      ...tuya.fingerprint("v90ladg\u0000", ["_TYST11_wv90ladg"]),
      ...tuya.fingerprint("TS0601", ["_TZE200_wv90ladg"]),
    ],
    model: "HT-08",
    vendor: "ETOP",
  ...
  {
    fingerprint: [
      ...tuya.fingerprint("dpplnsn\u0000", ["_TYST11_2dpplnsn"]),
      ...tuya.fingerprint("TS0601", ["_TZE200_2dpplnsn"]),
    ],
    model: "HT-10",
    vendor: "ETOP",
  ...
  },
  {
    ...
    description: "Thermostatic radiator valve",
    whiteLabel: [
      tuya.whitelabel("id3", "GTZ06", "Thermostatic radiator valve", [
        "_TZE200_z1tyspqw",
      ]),
      tuya.whitelabel("AVATTO", "TRV07", "Thermostatic radiator valve", [
        "_TZE200_bvrlmajk",
      ]),
    ],
    onEvent: tuya.onEventSetLocalTime,
    ...
    exposes: [
      ...
      e.window_detection(),
      e
        .binary("window", ea.STATE, "CLOSE", "OPEN")
      ...
      e
        .numeric("boost_time", ea.STATE_SET)
        .withUnit("min")
        .withDescription("Countdown in minutes")
        .withValueMin(0)
        .withValueMax(1000),
    ],
    meta: {
      tuyaDatapoints: [
        ...
        [
          1,
          "system_mode",
          tuya.valueConverter.thermostatSystemModeAndPreset("system_mode"),
        ],
        [
          1,
          "preset",
          tuya.valueConverter.thermostatSystemModeAndPreset("preset"),
        ],
        ...
        [
          6,
          "running_state",
          tuya.valueConverterBasic.lookup({ heat: 1, idle: 0 }),
        ],
  ...
  },
  {
    ...
    vendor: "Tuya",
    whiteLabel: [
      tuya.whitelabel("Sber", "SBDV-00185", "Thermostatic radiator valve", [
        "_TZE204_cvcu2p6e",
      ]),
    ],
    description: "Thermostatic radiator valve.",
    ...
    exposes: [
      ...
      e.window_detection(),
      e
        .binary("window", ea.STATE, "OPEN", "CLOSE")
        .withDescription("Window status closed or open "),
      e
        .binary("alarm_switch", ea.STATE, "ON", "OFF")
        .withDescription("Thermostat in error state"),
  ...
  },
  {
    ...
    description: "Thermostatic radiator valve.",
    whiteLabel: [
      tuya.whitelabel("Moes", "TRV801", "Thermostatic radiator valve", [
        "_TZE204_9mjy74mp",
        "_TZE200_9mjy74mp",
      ]),
    ],
    ...
    exposes: [
      ...
      e
        .enum("screen_orientation", ea.STATE_SET, [
          "up",
          "right",
          "down",
          "left",
  ...
  },
  {
    ...
    description: "Thermostatic radiator valve.",
    whiteLabel: [
      tuya.whitelabel("Moes", "TRV801Z", "Thermostatic radiator valve", [
        "_TZE204_qyr2m29i",
      ]),
    ],
    ...
    exposes: [
      ...
      e
        ...
        .withValueMin(5)
        .withValueMax(30)
        .withDescription("Antifreeze mode temperature"),
      e
        .enum("display_brightness", ea.STATE_SET, ["high", "medium", "low"])
        .withDescription("Display brightness"),
      e
    ...
    meta: {
      tuyaDatapoints: [
        ...
        [
          102,
          "schedule_monday",
          tuya.valueConverter.thermostatScheduleDayMultiDP_TRV602Z_WithDayNumber(
            1,
          ),
        ],
        [
  ...
  },
  {
    ...
    exposes: [
      ...
      e.child_lock(),
      e
        .binary("system_mode", ea.STATE_SET, "Auto", "Manual")
        .withDescription("Manual = Manual or Schedule = Auto"),
      e.eco_mode(),
      e
        .temperature_sensor_select(["IN", "AL", "OU"])
        .withLabel("Sensor")
        .withDescription("Choose which sensor to use. Default: AL"),
      e
        .enum("valve_state", ea.STATE, ["close", "open"])
      ...
      e
        .enum("working_day", ea.STATE_SET, ["disabled", "5-2", "6-1", "7"])
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withDescription("Recover state after power outage"),
      e
        .enum("indicator_mode", ea.ALL, ["off", "off/on", "on/off"])
        .withDescription("LED indicator mode"),
    ],
    onEvent: (type, data, device, options) =>
      tuya.onEventMeasurementPoll(type, data, device, options, true, false),
  },
  {
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel(
        "Moes",
        "ZK-EU",
        "Smart wallsocket (with power monitoring)",
        ["_TZ3000_ss98ec5d"],
      ),
      tuya.whitelabel("Elivco", "LSPA9", "Smart plug (with power monitoring)", [
        "_TZ3000_okaz9tjs",
      ]),
      tuya.whitelabel(
        "PSMART",
      ...
      tuya.whitelabel("Nous", "A6Z", "Outdoor smart socket", [
        "_TZ3000_266azbg3",
      ]),
      tuya.whitelabel(
        "Nedis",
      ...
      tuya.whitelabel("Nous", "A9Z", "Smart ZigBee Socket", [
        "_TZ3210_ddigca5n",
      ]),
    ],
    ota: true,
    extend: [
      tuya.modernExtend.tuyaOnOff({
        ...
        electricalMeasurementsFzConverter:
          fzLocal.TS011F_electrical_measurement,
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      if (
        !["_TZ3000_0zfrhq4i", "_TZ3000_okaz9tjs", "_TZ3000_typdpbpg"].includes(
          device.manufacturerName,
        )
      ) {
      ...
      const acCurrentDivisor =
        device.manufacturerName === "_TZ3000_typdpbpg" ? 2000 : 1000;
      endpoint.saveClusterAttributeKeyValue("haElectricalMeasurement", {
        acCurrentDivisor,
        acCurrentMultiplier: 1,
      });
  ...
  },
  {
    ...
    onEvent: (type, data, device, options) =>
      tuya.onEventMeasurementPoll(
        ...
        [100, 160].includes(device.applicationVersion) ||
          ["1.0.5\u0000"].includes(device.softwareBuildID), // polling for energy
      ),
  },
  {
    fingerprint: tuya.fingerprint("TS011F", [
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Nous", "E8", "Smoke sensor", [
        "_TZE200_0zaf1cr8",
        "_TZE284_0zaf1cr8",
      ]),
    ],
  ...
  },
  {
    ...
    exposes: [
      ...
      e.test(),
      e
        .numeric("smoke_concentration", ea.STATE)
        .withUnit("ppm")
        .withDescription("Parts per million of smoke detected"),
      e
        .binary("device_fault", ea.STATE, true, false)
        .withDescription("Indicates a fault with the device"),
    ],
    meta: {
      tuyaDatapoints: [
        [1, "smoke", tuya.valueConverter.trueFalse0],
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withDescription("Enable the alarm"),
    ],
    meta: {
      tuyaDatapoints: [
        [1, "smoke", tuya.valueConverter.trueFalse0],
  ...
  },
  {
    ...
    toZigbee: [
      legacy.toZigbee.SA12IZL_silence_siren,
      legacy.toZigbee.SA12IZL_alarm,
    ],
    exposes: [
      e.battery(),
      e
        .binary("smoke", ea.STATE, true, false)
        .withDescription("Smoke alarm status"),
      e
        .enum("battery_level", ea.STATE, ["low", "middle", "high"])
        .withDescription("Battery level state"),
      e
        .binary("alarm", ea.STATE_SET, true, false)
        .withDescription("Enable the alarm"),
      e
        .binary("silence_siren", ea.STATE_SET, true, false)
        .withDescription("Silence the siren"),
    ],
    onEvent: tuya.onEventSetTime,
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Tuya", "PJ-1203-W", "Electricity energy monitor", [
        "_TZE284_cjbofhxw",
      ]),
    ],
  },
  {
    ...
    exposes: [
      ...
      e.power_reactive(),
      e
        .numeric("energy_reactive", ea.STATE)
        .withUnit("kVArh")
        .withDescription("Sum of reactive energy"),
      e
        .numeric("total_energy", ea.STATE)
        .withUnit("kWh")
        .withDescription("Total consumed and produced energy"),
    ],
    meta: {
      tuyaDatapoints: [
  ...
  },
  {
    ...
    exposes: [
      ...
      e.temperature(),
      e
        .binary("leakage_test", ea.STATE_SET, "ON", "OFF")
        .withDescription("Turn ON to perform a leagage test"),
      e
        .binary("over_current_breaker", ea.STATE_SET, "ON", "OFF")
        .withDescription(
          "OFF - alarm only, ON - relay will turn off when threshold reached",
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .numeric("threshold_2_value", ea.STATE)
        .withDescription("Setup value on the device"),
      e
  ...
  },
  {
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      await device
        .getEndpoint(1)
        .read("genOnOff", ["onOff", "moesStartUpOnOff"]);
  ...
  },
  {
    ...
    extend: [
      tuya.modernExtend.tuyaOnOff({ backlightModeOffNormalInverted: true }),
    ],
    whiteLabel: [
      ...
      tuya.whitelabel("Nous", "LZ3", "Smart water/gas valve", [
        "_TZ3000_abjodzas",
      ]),
    ],
    configure: async (device, coordinatorEndpoint) => {
  ...
  {
    fingerprint: tuya.fingerprint("TS0011", [
      "_TZ3000_qmi1cfuq",
      "_TZ3000_txpirhfq",
      "_TZ3000_ji4araar",
    ]),
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("Moes", "ZS-EUB_2gang", "Smart light switch - 2 gang", [
        "_TZ3000_18ejxno0",
      ]),
    ],
    extend: [
  ...
  {
    fingerprint: tuya.fingerprint("TS0012", [
      "_TZ3000_jl7qyupf",
      "_TZ3000_nPGIPl5D",
      "_TZ3000_kpatq5pq",
      "_TZ3000_ljhbw1c9",
    ...
    whiteLabel: [
      ...
      tuya.whitelabel(
        ...
        ["_TZ3000_kpatq5pq", "_TZ3000_ljhbw1c9"],
      ),
    ],
    extend: [
      tuya.modernExtend.tuyaOnOff({
  ...
  {
    fingerprint: tuya.fingerprint("TS0013", [
      "_TZ3000_ypgri8yz",
      "_TZ3000_sznawwyw",
    ]),
    ...
    whiteLabel: [
      ...
      tuya.whitelabel(
        "Girier",
        "ZB08",
        "3 Channel Switch Module-L - (No Neutral Wire)",
        ["_TZ3000_ypgri8yz"],
      ),
    ],
    extend: [
  ...
  },
  {
    ...
    toZigbee: [
      ...
      legacy.toZigbee.tuya_dimmer_level,
    ],
    exposes: [
      e
        .light_brightness()
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("MHCOZY", "TYWB 4ch-RF", "4 channel relay", [
        "_TZ3000_u3oupgdy",
        "_TZ3000_imaccztn",
      ]),
      tuya.whitelabel(
  ...
  },
  {
    ...
    fromZigbee: [
      fz.on_off,
      tuya.fz.power_on_behavior_2,
      fz.ignore_basic_report,
      fzLocal.TS0726_action,
    ],
    toZigbee: [
      tz.on_off,
      tuya.tz.power_on_behavior_2,
      tzLocal.TS0726_switch_mode,
    ],
    exposes: [
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel(
        "AVATTO",
        "TS0006_1",
        "4 gang switch module with neutral wire and socket",
        ["_TZ3000_cvis4qmw"],
      ),
    ],
  },
  ...
  },
  {
    ...
    exposes: [
      e
        ...
        .withLocalTemperature(),
      e
        .binary("frost", ea.STATE_SET, "ON", "OFF")
        .withDescription("Antifreeze function"),
    ],
    meta: {
      tuyaDatapoints: [
  ...
  },
  {
    ...
    fromZigbee: [
      fz.ignore_basic_report,
      fz.ignore_tuya_set_time,
      legacy.fromZigbee.x5h_thermostat,
    ],
    ...
    exposes: [
      ...
      e.week(),
      e
        .enum("brightness_state", ea.STATE_SET, [
          "off",
          "low",
          "medium",
          "high",
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .enum("backlight_mode", ea.STATE_SET, ["off", "low", "medium", "high"])
        .withDescription("Intensity of the backlight"),
      e
        .enum("working_day", ea.STATE_SET, ["disabled", "6-1", "5-2", "7"])
        .withDescription("Workday setting"),
      e
        .text("schedule_weekday", ea.STATE_SET)
        .withDescription("Workdays (6 times `hh:mm/cc.c°C`)"),
      e
    ...
    meta: {
      tuyaDatapoints: [
        ...
        [
          110,
          "backlight_mode",
          tuya.valueConverter.backlightModeOffLowMediumHigh,
        ],
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withLocalTemperatureCalibration(-9.9, 9.9, 0.1, ea.STATE_SET),
      e
        .enum("backlight_mode", ea.STATE_SET, [
          "off",
          "always_low",
          "always_mid",
          "always_high",
  ...
  },
  {
    ...
    configure: tuya.configureMagicPacket,
    whiteLabel: [
      tuya.whitelabel("Moes", "ZSS-QT-LS-C", "Light sensor", [
        "_TZ3000_9kbbfeho",
      ]),
    ],
    exposes: [e.battery()],
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Niceboy", "ORBIS Vibration Sensor", "Vibration sensor", [
        "_TYZB01_821siati",
      ]),
      tuya.whitelabel("iHseno", "_TZ3000_lzdjjfss", "Vibration sensor", [
        "_TZ3000_lzdjjfss",
      ]),
    ],
  ...
  },
  {
    ...
    toZigbee: [
      tz.on_off,
      tuya.tz.power_on_behavior_1,
      tz.tuya_relay_din_led_indicator,
    ],
    ...
    exposes: [
      ...
      e
        ...
        .withDescription("Recover state after power outage"),
      e
        .enum("indicator_mode", ea.STATE_SET, ["off", "on_off", "off_on"])
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel(
        "Nous",
        "DZ",
        "DIN Switch (with power monitoring via polling)",
        ["_TZ3000_2iiimqs9"],
      ),
    ],
    ota: true,
  ...
  },
  {
    ...
    fromZigbee: [
      legacy.fromZigbee.matsee_garage_door_opener,
      fz.ignore_basic_report,
    ],
    toZigbee: [
      legacy.toZigbee.matsee_garage_door_opener,
      legacy.toZigbee.tuya_data_point_test,
    ],
    ...
    exposes: [
      e
        .binary("trigger", ea.STATE_SET, true, false)
        .withDescription("Trigger the door movement"),
      e
  ...
  {
    fingerprint: tuya.fingerprint("TS0603", [
      "_TZE608_c75zqghm",
      "_TZE608_fmemczv1",
    ]),
    model: "TS0603",
  ...
  },
  {
    ...
    fromZigbee: [
      ...
      fz.temperature,
      fz.humidity,
      fz.ts0201_temperature_humidity_alarm,
    ],
    ...
    exposes: [
      ...
      e
        ...
        .withValueMax(80),
      e
        .numeric("alarm_humidity_max", ea.STATE_SET)
        .withUnit("%")
        .withDescription("Alarm humidity max")
        .withValueMin(0)
        .withValueMax(100),
      e
        .numeric("alarm_humidity_min", ea.STATE_SET)
        .withUnit("%")
        .withDescription("Alarm humidity min")
        .withValueMin(0)
        .withValueMax(100),
      e
        .enum("alarm_humidity", ea.STATE, [
          "below_min_humdity",
          "over_humidity",
          "off",
        ])
      ...
      e
        .enum("alarm_temperature", ea.STATE, [
    ...
    exposes: [
      ...
      e
        .enum(
          "o_sensitivity",
          ea.STATE_SET,
          Object.values(legacy.msLookups.OSensitivity),
        )
        .withDescription("O-Sensitivity mode"),
      e
        .enum(
          "v_sensitivity",
          ea.STATE_SET,
          Object.values(legacy.msLookups.VSensitivity),
        )
        .withDescription("V-Sensitivity mode"),
      e
        .enum("led_status", ea.STATE_SET, ["ON", "OFF"])
        .withDescription("Led status switch"),
      e
        .numeric("vacancy_delay", ea.STATE_SET)
        .withUnit("sec")
        .withDescription("Vacancy delay")
        .withValueMin(0)
        .withValueMax(1000),
      e
        .numeric("light_on_luminance_prefer", ea.STATE_SET)
        .withDescription("Light-On luminance prefer")
        .withValueMin(0)
        .withValueMax(10000),
      e
        .numeric("light_off_luminance_prefer", ea.STATE_SET)
        .withDescription("Light-Off luminance prefer")
        .withValueMin(0)
        .withValueMax(10000),
      e
        .enum("mode", ea.STATE_SET, Object.values(legacy.msLookups.Mode))
        .withDescription("Working mode"),
      e.numeric("luminance_level", ea.STATE).withDescription("Luminance level"),
      e
        .numeric("reference_luminance", ea.STATE)
        .withDescription("Reference luminance"),
      e
        .numeric("vacant_confirm_time", ea.STATE)
        .withDescription("Vacant confirm time"),
    ],
  },
  {
    fingerprint: tuya.fingerprint("TS0601", [
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      await tuya.sendDataPointEnum(
        endpoint,
        legacy.dataPoints.trsfTumbleSwitch,
        0,
      );
      await tuya.configureMagicPacket(device, coordinatorEndpoint);
    },
    exposes: [
      ...
      e
        .enum("motion_direction", ea.STATE, [
          "standing_still",
          "moving_forward",
          "moving_backward",
        ])
      ...
      e
        .enum("tumble_switch", ea.STATE_SET, ["ON", "OFF"])
        .withDescription("Tumble status switch"),
      e
      ...
      e
        .enum("fall_down_status", ea.STATE, ["none", "maybe_fall", "fall"])
        .withDescription("Fall down status"),
      e
        .text("static_dwell_alarm", ea.STATE)
        .withDescription("Static dwell alarm"),
    ],
    meta: {
      tuyaDatapoints: [
        ...
        [
          116,
          "fall_down_status",
          tuya.valueConverterBasic.lookup({ none: 0, maybe_fall: 1, fall: 2 }),
        ],
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .numeric("countdown_timer", ea.STATE_SET)
        .withValueMin(0)
        .withValueMax(86400)
        .withUnit("s")
        .withDescription("Countdown timer"),
      e
        .numeric("voltage_rms", ea.STATE)
        .withUnit("V")
        .withDescription("Voltage RMS"),
      e.numeric("current", ea.STATE).withUnit("A").withDescription("Current"),
      e
        .numeric("current_average", ea.STATE)
        .withUnit("A")
        .withDescription("Current average"),
      e.power(),
      e.voltage(),
      e.energy(),
      e.temperature(),
      e
        .numeric("power_l1", ea.STATE)
        .withUnit("W")
        .withDescription("Instantaneous measured power on phase 1"),
      e
        .numeric("power_l2", ea.STATE)
        .withUnit("W")
        .withDescription("Instantaneous measured power on phase 2"),
      e
        .numeric("power_l3", ea.STATE)
        .withUnit("W")
        .withDescription("Instantaneous measured power on phase 3"),
      e
        .numeric("energy_consumed", ea.STATE)
        .withUnit("kWh")
        .withDescription("Consumed energy"),
      e
        .enum("clear_device_data", ea.SET, [""])
        .withDescription("Clear device data"),
    ],
  },
  {
    fingerprint: tuya.fingerprint("TS004F", [
    ...
    whiteLabel: [
      tuya.whitelabel("Tuya", "ZG-101Z_D", "Smart knob", ["_TZ3000_402vrq2i"]),
    ],
    toZigbee: [tz.tuya_operation_mode],
    exposes: [
      e.action([
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Tuya", "FS-05R", "Mini dimmable switch 1 channel", [
        "_TZ3000_mgusv51k",
      ]),
    ],
  },
  {
    fingerprint: tuya.fingerprint("TS0052", [
      "_TZ3000_zjtxnoft",
      "_TZ3000_kvwrdf47",
    ]),
    model: "TS0052_2",
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Tuya", "ZY-M100-L", "Ceiling human breathe sensor", [
        "_TZE204_ztc6ggyl",
      ]),
      tuya.whitelabel("Moes", "ZSS-QY-HP", "Human presence sensor", [
        "_TZE204_fwondbzy",
      ]),
    ],
    exposes: [
      ...
      e
        .enum(
          "self_test",
          ea.STATE,
          Object.values(legacy.tuyaHPSCheckingResult),
        )
        .withDescription(
  ...
  },
  {
    ...
    toZigbee: [tuya.tz.datapoints],
    whiteLabel: [
      tuya.whitelabel("Wenzhi", "WZ-M100-W", "Human presence sensor", [
        "_TZE204_e5m9c5hl",
      ]),
    ],
    exposes: [
  ...
  },
  {
    ...
    exposes: [
      e.presence(),
      e
        .enum("motion_state", ea.STATE, ["none", "large", "small", "static"])
        .withDescription("Motion state"),
      e.illuminance(),
      e
        .numeric("fading_time", ea.STATE_SET)
        .withValueMin(0)
      ...
      e
        .enum("mode", ea.STATE_SET, ["off", "arm", "alarm", "doorbell"])
        .withDescription("Working mode"),
      e
        .enum("alarm_volume", ea.STATE_SET, ["mute", "low", "medium", "high"])
      ...
      e
        ...
        .withDescription("Alarm time"),
      e
        .binary("light_mode", ea.STATE_SET, "ON", "OFF")
        .withDescription("LED indicator mode"),
    ],
    meta: {
      tuyaDatapoints: [
  ...
  },
  {
    ...
    fromZigbee: [
      legacy.fromZigbee.nous_lcd_temperature_humidity_sensor,
      fz.ignore_tuya_set_time,
    ],
    ...
    exposes: [
      ...
      e
        .enum("temperature_unit_convert", ea.STATE_SET, [
          "celsius",
          "fahrenheit",
        ])
        .withDescription("Current display unit"),
      e
        .enum("temperature_alarm", ea.STATE, [
          "canceled",
          "lower_alarm",
      ...
      e
        .numeric("max_humidity", ea.STATE_SET)
        .withUnit("%")
        .withValueMin(0)
        .withValueMax(100)
        .withDescription("Alarm humidity max"),
      e
        .numeric("min_humidity", ea.STATE_SET)
        .withUnit("%")
        .withValueMin(0)
        .withValueMax(100)
        .withDescription("Alarm humidity min"),
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .numeric("cycle_time", ea.STATE)
        .withDescription("Cycle time")
        .withUnit("ms"),
      e
        .enum("top_limit", ea.STATE_SET, ["SET", "CLEAR"])
        .withDescription("Setup or clear top limit"),
      e
        .enum("bottom_limit", ea.STATE_SET, ["SET", "CLEAR"])
        .withDescription("Setup or clear bottom limit"),
      e
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Tuya", "UFO-R4Z", "Universal smart IR remote control", [
        "_TZ3290_rlkmy85q4pzoxobl",
      ]),
  ...
  },
  {
    ...
    exposes: [
      e.presence(),
      e
        .numeric("duration_of_attendance", ea.STATE)
        .withUnit("min")
        .withDescription("Shows the presence duration in minutes"),
      e
        .numeric("duration_of_absence", ea.STATE)
        .withUnit("min")
        .withDescription("Shows the duration of the absence in minutes"),
      e
        .binary("led_state", ea.STATE_SET, true, false)
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("HOBEIAN", "ZG-227Z", "Temperature and humidity sensor", [
        "_TZE200_a8sdabtg",
        "_TZE200_vs0skpuc",
      ]),
      tuya.whitelabel(
  ...
  },
  {
    ...
    exposes: [
      ...
      e.battery_low(),
      e
        .binary("vacation", ea.STATE_SET, "ON", "OFF")
        .withDescription("Vacation mode"),
      e.enum("alarm", ea.STATE, ["ALARM", "IDLE"]).withDescription("Alarm"),
      e
        .binary("alarm_switch", ea.STATE_SET, "ON", "OFF")
        .withDescription("Alarm enable"),
      e
        .binary("handlesound", ea.STATE_SET, "ON", "OFF")
        .withDescription("Handle closed sound"),
      e
        .enum("opening_mode", ea.STATE, ["closed", "tilted"])
        .withDescription("Window tilt"),
      e.temperature(),
      e.humidity(),
      e
        .binary("keysound", ea.STATE_SET, "ON", "OFF")
  ...
  {
    fingerprint: [
      ...tuya.fingerprint("TS0601", ["_TZE200_f1pvdgoh", "_TZE200_me6wtiqs"]),
    ...
    whiteLabel: [
      tuya.whitelabel(
        "Immax",
        "07527L",
        "NEO Smart PIR sensor 2v1 Zigbee 3.0",
        ["_TZE200_me6wtiqs"],
      ),
    ],
    meta: {
  ...
  {
    fingerprint: [
      ...tuya.fingerprint("TS0601", ["_TZE200_8isdky6j"]),
      ...tuya.fingerprint("TS0225", ["_TZE200_p6fuhvez"]),
    ],
    model: "ZG-225Z",
    vendor: "Tuya",
    description: "Gas sensor",
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withDescription("State of the motion"),
      e
        .numeric("target_distance", ea.STATE)
        .withDescription("Distance to target")
        .withUnit("m"),
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withDescription("LED indicator mode"),
      e
        .enum("motion_detection_mode", ea.STATE_SET, [
          "only_pir",
          "pir_and_radar",
          "only_radar",
        ])
  ...
  },
  {
    ...
    toZigbee: [
      tz.TS110E_light_onoff_brightness,
      tuya.tz.power_on_behavior_1,
      tz.TS110E_options,
    ],
    exposes: [
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel(
        "Lonsonho",
        "QS-Zigbee-D02-TRIAC-L_1",
        "1 channel dimmer",
        ["_TZ3210_weaqkhab"],
      ),
      tuya.whitelabel(
        "Lonsonho",
        "QS-Zigbee-D02-TRIAC-LN_1",
        "1 channel dimmer",
        ["_TZ3210_ngqk6jia"],
      ),
    ],
    ...
    fromZigbee: [
      fz.TS110E,
      fz.TS110E_light_type,
      tuya.fz.power_on_behavior_1,
      fz.on_off,
    ],
    toZigbee: [
      tz.TS110E_onoff_brightness,
      tz.TS110E_options,
      tuya.tz.power_on_behavior_1,
    ...
    exposes: (device, options) => {
      ...
      const countdownValueStep =
        device?.manufacturerName === "_TZ3210_ngqk6jia" ? 30 : 1;
      return [
  ...
  },
  {
    ...
    description: "2 channel dimmer",
    whiteLabel: [
      tuya.whitelabel("Nedis", "ZBWD20RD", "SmartLife Triac Dimmer", [
        "_TZ3210_mt5xjoy6",
      ]),
    ],
    fromZigbee: [
  ...
  },
  {
    ...
    exposes: [
      e
        .enum("valve_status", ea.STATE, ["manual", "auto", "idle"])
        .withDescription("Valve 1 status (manual, auto, idle)")
        .withEndpoint("l1"),
      e
        .enum("valve_status", ea.STATE, ["manual", "auto", "idle"])
        .withDescription("Valve 2 status (manual, auto, idle)")
        .withEndpoint("l2"),
      e
        .switch()
        .withEndpoint("l1")
        .withDescription("Valve 1 on/off")
        .withLabel("Valve 1"),
      e
        .switch()
        .withEndpoint("l2")
        .withDescription("Valve 2 on/off")
        .withLabel("Valve 2"),
      e
      ...
      e
        ...
        .withEndpoint("l2"),
      e
        .numeric("valve_duration", ea.STATE)
        .withUnit("s")
        .withDescription("Valve 1 irrigation last duration in seconds")
        .withEndpoint("l1"),
      e
        .numeric("valve_duration", ea.STATE)
        .withUnit("s")
        .withDescription("Valve 2 irrigation last duration in seconds")
        .withEndpoint("l2"),
      e.battery(),
      e.battery_voltage(),
    ],
    meta: {
      ...
      tuyaDatapoints: [
        ...
        [
          104,
          "valve_status_l1",
          tuya.valueConverterBasic.lookup({ manual: 0, auto: 1, idle: 2 }),
        ], // Valve 1 status
        [
          105,
          "valve_status_l2",
  ...
  },
  {
    ...
    exposes: [
      ...
      tuya.exposes
        ...
        .withValueMax(43200)
        .withUnit("s")
        .withDescription("Max ON time in seconds"),
      e
  ...
  },
  {
    ...
    exposes: [
      e
        .binary("status_indication", ea.STATE_SET, "ON", "OFF")
        .withDescription("Light switch"),
      tuya.exposes.switch(),
      e.power_on_behavior(["OFF", "ON"]).withAccess(ea.STATE_SET),
      e
        .enum("fan_speed", ea.STATE_SET, ["minimum", "medium", "maximum"])
  ...
  },
  {
    ...
    exposes: [
      e.warning(),
      e
        .binary("light", ea.STATE_SET, "ON", "OFF")
        .withDescription("Turn the light of the alarm ON/OFF"),
      e
        .numeric("duration", ea.STATE_SET)
        .withValueMin(60)
        .withValueMax(3600)
        ...
        .withDescription("Duration of the alarm"),
      e
        .enum("volume", ea.STATE_SET, ["mute", "low", "medium", "high"])
  ...
  },
  {
    ...
    exposes: (device, options) => {
      ...
      if (
        !["_TZ3000_303avxxt", "_TZ3000_zjchz7pd"].includes(
          device?.manufacturerName,
        )
      ) {
        exposes.push(
          ...
          e
            .binary("temperature_breaker", ea.STATE_SET, "ON", "OFF")
      ...
      exposes.push(
        ...
        e
          .binary("power_breaker", ea.STATE_SET, "ON", "OFF")
    ...
    whiteLabel: [
      tuya.whitelabel("Tongou", "TO-Q-SY2-163JZT", "Smart circuit breaker", [
        "_TZ3000_cayepv1a",
      ]),
      tuya.whitelabel("EARU", "EAKCB-T-M-Z", "Smart circuit breaker", [
        "_TZ3000_lepzuhto",
      ]),
      tuya.whitelabel(
        "EARU",
        "EAYCB-Z-2P",
        "Smart circuit breaker with leakage protection",
        ["_TZ3000_zrm3oxsh"],
      ),
      tuya.whitelabel("UNSH", "SMKG-1KNL-EU-Z", "Smart circuit Breaker", [
        "_TZ3000_qystbcjg",
      ]),
      tuya.whitelabel("Tomzn", "TOB9Z-VAP", "Smart circuit breaker", [
        "_TZ3000_303avxxt",
      ]),
      tuya.whitelabel("Immax", "07573L", "Smart circuit breaker", [
        "_TZ3000_zjchz7pd",
      ]),
    ],
  },
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("RoomsAI", "37022474_2", "3 Gang switch with backlight", [
        "_TZ3000_avky2mvc",
      ]),
      tuya.whitelabel(
        "Nova Digital",
      ...
      tuya.whitelabel(
        "Nova Digital",
        "FZB-3",
        "3 Gang physical switch with backlight",
        ["_TZ3000_qxcnwv26"],
      ),
    ],
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Homeetec", "37022714", "4 Gang switch with backlight", [
        "_TZE200_hewlydpz",
      ]),
    ],
  },
  {
    ...
    exposes: [
      e.cover_position().setAccess("position", ea.STATE_SET),
      e
        .enum("calibration", ea.STATE_SET, ["START", "END"])
        .withDescription("Calibration"),
      e
        .binary("backlight_mode", ea.STATE_SET, "ON", "OFF")
        .withDescription("Backlight"),
      e
        .enum("motor_steering", ea.STATE_SET, ["FORWARD", "BACKWARD"])
        .withDescription("Motor Steering"),
      e
        .binary("child_lock", ea.STATE_SET, "ON", "OFF")
        .withDescription("Child Lock"),
    ],
    ...
    whiteLabel: [
      tuya.whitelabel("Homeetec", "37022483", "Curtain/blind switch", [
        "_TZE200_p6vz3wzt",
      ]),
    ],
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Lidl", "HG09648", "Livarno roller blinds", [
        "_TZB000_42ha4rsc",
      ]),
    ],
    exposes: [
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withDescription("Timestamp at power measure (phase b)"),
      e
  ...
  },
  {
    ...
    exposes: [
      ...
      e.current(),
      e
        .enum("energy_flow", ea.STATE, ["consuming", "producing"])
        .withDescription("Direction of energy"),
    ],
    meta: {
      tuyaDatapoints: [
        [1, "energy", tuya.valueConverter.divideBy100],
        ...
        [
          102,
          "energy_flow",
          tuya.valueConverterBasic.lookup({ consuming: 0, producing: 1 }),
        ],
      ],
    },
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Nova Digital", "ZTS-8W-B", "8 Gang Switch", [
        "_TZE204_nvxorhcj",
      ]),
    ],
  },
  ...
  },
  {
    ...
    exposes: [
      ...Array.from({ length: 10 }, (_, i) =>
        tuya.exposes.switch().withEndpoint(`l${i + 1}`),
      ),
    ],
    endpoint: (device) => {
      return {
  ...
  // TS011F
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel("KTNNKG", "ZB1248-10A", "Relay switch", [
        "_TZ3000_8fdayfch",
      ]),
      tuya.whitelabel("UseeLink", "SM-AZ713", "Smart water/gas valve", [
        "_TZ3000_tvuarksa",
      ]),
    ],
  },
  {
    ...
    onEvent: (type, data, device, options) =>
      tuya.onEventMeasurementPoll(type, data, device, options, true, false),
    whiteLabel: [
      tuya.whitelabel("Nous", "A4Z", "2 gang outdoor plug", [
        "_TZ3000_rqbjepe8",
        "_TZ3000_uwkja6z1",
      ]),
    ],
  },
  {
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      for (const ID of [1, 2, 3, 4, 5]) {
        await reporting.bind(device.getEndpoint(ID), coordinatorEndpoint, [
          "genOnOff",
        ]);
      }
    },
    whiteLabel: [
      tuya.whitelabel("UseeLink", "SM-0306E-2W", "4 gang switch, with USB", [
        "_TZ3000_cfnprab5",
      ]),
      tuya.whitelabel(
        "UseeLink",
        "SM-O301-AZ",
        "AU 4 plug 10A power board + USB",
        ["_TZ3000_o005nuxx"],
      ),
    ],
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Cleverio", "SS200", "Motion sensor", [
        "_TYZB01_z2umiwvq",
      ]),
      tuya.whitelabel("Marmitek", "SM0202_1", "Motion sensor", [
        "_TYZB01_yr95mpib",
      ]),
    ],
  },
  {
    ...
    exposes: [
      ...
      e
        .binary("touch", ea.STATE_SET, "ON", "OFF")
        .withDescription("Touch control"),
    ],
    configure: async (device, coordinatorEndpoint) => {
      await tuya.configureMagicPacket(device, coordinatorEndpoint);
      await reporting.bind(device.getEndpoint(1), coordinatorEndpoint, [
    ...
    meta: {
      ...
      tuyaDatapoints: [
        ...
        [
          0x6b,
          "touch",
          tuya.valueConverterBasic.lookup({ ON: true, OFF: false }),
        ],
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel(
        "Nova Digital",
        "FZB-4",
        "Interruptor de 4 canais com backlight e neutro",
        ["TZE204_unsxl4ir"],
      ),
    ],
  },
  {
    ...
    exposes: [
      ...[1, 2, 3, 4, 5, 6, 7, 8].map((i) =>
        tuya.exposes.switch().withEndpoint(`l${i}`),
      ),
      ...[1, 2, 3, 4, 5, 6, 7, 8].map((i) =>
        e.power_on_behavior().withAccess(ea.STATE_SET).withEndpoint(`l${i}`),
      ),
      ...[1, 2, 3, 4, 5, 6, 7, 8].map((i) =>
        tuya.exposes.switchMode().withEndpoint(`l${i}`),
      ),
      ...[1, 2, 3, 4, 5, 6, 7, 8].map((i) =>
        tuya.exposes.lightMode().withEndpoint(`l${i}`),
      ),
      tuya.exposes.backlightModeOffOn().withAccess(ea.STATE_SET),
      e.action([
        "scene_1",
      ...
      e.presence(),
      new exposes.Numeric("delay", ea.STATE_SET)
        .withUnit("sec")
        .withDescription("light off delay")
        .withValueMin(0)
        .withValueMax(1000),
    ],
    endpoint: (device) => {
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel(
        "Ltech",
        "TY-12-100-400-W1Z",
        "12W 100-400mA Zigbee CC Dimmable LED driver",
        ["_TZE200_86nbew0j"],
      ),
      tuya.whitelabel(
        "Ltech",
        "TY-75-24-G2Z2",
        "150W 24V Zigbee CV tunable white LED driver",
        ["_TZE200_io0zdqh1"],
      ),
      tuya.whitelabel(
        "Lifud",
        "LF-AAZ012-0400-42",
        "Zigbee dimmable LED driver 4-40W 220-240Vac",
        ["_TZE200_drs6j6m5"],
      ),
      tuya.whitelabel(
        "Lifud",
        "LF-GAZ150A6250-24",
        "Lifud Zigbee LED Driver CCT 150W 24V",
        ["_TZE200_ywe90lt0"],
      ),
    ],
  },
  {
    ...
    exposes: [
      e
        ...
        .withDescription("Turn the light of the alarm ON/OFF"),
      e
        .enum("type", ea.STATE_SET, ["sound", "light", "sound+light", "normal"])
        .withDescription("Alarm type"),
      e
        .enum("volume", ea.STATE_SET, ["mute", "low", "middle", "high"])
        .withDescription("Volume of the alarm"),
      e
        ...
        .withDescription("Ringtone of the alarm"),
      e
        .enum("power_type", ea.STATE, ["battery", "cable"])
        .withDescription("Power type"),
      e
        .numeric("duration", ea.STATE_SET)
        .withValueMin(1)
        .withValueMax(60)
    ...
    meta: {
      tuyaDatapoints: [
        ...
        [
          6,
          "power_type",
          tuya.valueConverterBasic.lookup({ cable: false, battery: true }),
        ],
  ...
  },
  {
    ...
    configure: tuya.configureMagicPacket,
    exposes: [
      ...Array.from(Array(24).keys()).map((ep) =>
        tuya.exposes.switch().withEndpoint(`l${ep + 1}`),
      ),
    ],
    endpoint: (device) => {
      return {
    ...
    meta: {
      ...
      tuyaDatapoints: [
        ...
        [
          105,
          "state",
          tuya.valueConverterBasic.lookup({ none: 0, presence: 1, move: 2 }),
        ],
      ],
    },
  },
  ...
  },
  {
    ...
    exposes: (device, options) => {
      const exps = [
        ...
        e
          .binary("find_switch", ea.STATE_SET, "ON", "OFF")
          .withDescription("distance switch"),
        e.illuminance().withDescription("Illuminance sensor"),
        e
          .numeric("move_sensitivity", ea.STATE_SET)
          .withValueMin(1)
      ...
      if (
        !device ||
        device.manufacturerName === "_TZE204_gkfbdvyx" ||
        device.manufacturerName === "_TZE200_gkfbdvyx"
      ) {
        exps.push(
          e
            .numeric("detection_distance_min", ea.STATE_SET)
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .binary("light_linkage", ea.STATE_SET, true, false)
        .withDescription("Light linkage"),
      e
      ...
      e
        .enum("indicator_light", ea.STATE_SET, ["presence", "off", "on"])
        .withDescription("Controls when the indicator light is turned on"),
      e
    ...
    meta: {
      tuyaDatapoints: [
        ...
        [
          ...
          tuya.valueConverter.trueFalseEnum1,
        ],
  ...
  },
  {
    ...
    whiteLabel: [
      ...
      tuya.whitelabel(
        "Tuya",
        "MTG035-ZB-RL",
        "Human presence sensor with relay",
        ["_TZE204_pfayrzcw"],
      ),
      tuya.whitelabel(
        "Tuya",
        "MTG235-ZB-RL",
        "24G Human presence sensor with relay",
        ["_TZE204_clrdrnya", "_TZE200_clrdrnya"],
      ),
      tuya.whitelabel("QA", "QASZ24R", "mmWave 24 Ghz sensor with relay", [
        "_TZE284_4qznlkbu",
      ]),
    ],
    configure: tuya.configureMagicPacket,
  ...
  },
  {
    ...
    description: "Bidirectional energy meter with 80A current clamp",
    fromZigbee: [
      fzLocal.PJ1203A_strict_fz_datapoints,
      fzLocal.PJ1203A_sync_time_increase_seq,
    ],
    toZigbee: [tuya.tz.datapoints],
    onEvent: tuya.onEventSetTime,
    configure: tuya.configureMagicPacket,
    options: [
      e
        ...
        .withDescription(
          "Delay channel A publication until the next energy flow update (default false).",
        ),
      e
        .binary("late_energy_flow_b", ea.SET, true, false)
        .withDescription(
          "Delay channel B publication until the next energy flow update (default false).",
        ),
      e
        .binary("signed_power_a", ea.SET, true, false)
        .withDescription(
          "Report energy flow direction for channel A using signed power (default false).",
        ),
      e
        .binary("signed_power_b", ea.SET, true, false)
        .withDescription(
          "Report energy flow direction for channel B using signed power (default false).",
        ),
    ],
    exposes: [
      e.ac_frequency(),
  ...
  },
  {
    ...
    exposes: (device, options) => {
      ...
      const exposes = [
        ...
        e
          .binary("eco_mode", ea.STATE_SET, "ON", "OFF")
          .withDescription("ECO mode ON/OFF"),
        e
          .max_temperature_limit()
          .withDescription(
        ...
        e
          ...
          .withValueStep(1),
        e
          .binary("valve", ea.STATE, "OPEN", "CLOSE")
          .withDescription("3-Way Valve State"),
        e
          .binary("manual_mode", ea.STATE_SET, "ON", "OFF")
          .withDescription("Manual = ON or Schedule = OFF"),
        ...tuya.exposes.scheduleAllDays(
          ea.STATE_SET,
          "HH:MM/C HH:MM/C HH:MM/C HH:MM/C HH:MM/C HH:MM/C",
        ),
  ...
  },
  {
    ...
    exposes: [
      e.occupancy(),
      e
        .enum("human_motion_state", ea.STATE, ["none", "small", "large"])
    ...
    meta: {
      tuyaDatapoints: [
        ...
        [
          11,
          "human_motion_state",
          tuya.valueConverterBasic.lookup({ none: 0, small: 1, large: 2 }),
        ],
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel(
        "Tuya",
        "YK-S03",
        "Smart pH and Chlorine Tester for Swimming Pool",
        ["_TZE200_d9mzkhoq"],
      ),
    ],
    fromZigbee: [tuya.fz.datapoints],
    ...
    exposes: [
      e
        ...
        .withUnit("ppm")
        .withDescription("Total Dissolved Solids"),
      e.temperature(),
      e.battery(),
      e
        .numeric("ph", ea.STATE)
      ...
      e
        .numeric("ec", ea.STATE)
        .withUnit("µS/cm")
        .withDescription("Electrical conductivity"),
      e
      ...
      e
        .numeric("ph_max", ea.STATE_SET)
        .withUnit("pH")
        .withDescription("pH maximal value")
        .withValueMin(0)
        .withValueMax(20),
      e
        .numeric("ph_min", ea.STATE_SET)
        .withUnit("pH")
        .withDescription("pH minimal value")
        .withValueMin(0)
        .withValueMax(20),
      e
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("OWON", "THS317-ET-EY", "Temperature sensor with probe", [
        "_TZE200_01fvxamo",
      ]),
    ],
    meta: {
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("AVATTO", "ZWSM16-4-Zigbee", "4 gang switch module", [
        "_TZ3000_5ajpkyq6",
      ]),
    ],
  },
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withUnit("mA")
        .withDescription("Current leakage"),
      e
        .enum("event", ea.STATE, [
          "normal",
          "over_current_trip",
      ...
      e
        .enum("over_current_setting", ea.STATE_SET, ["Ignore", "Alarm", "Trip"])
      ...
      e
        ...
        .withValueMax(40),
      e
        .enum("over_voltage_setting", ea.STATE_SET, ["Ignore", "Alarm", "Trip"])
      ...
      e
        ...
        .withValueMax(295),
      e
        .enum("under_voltage_setting", ea.STATE_SET, [
          "Ignore",
          "Alarm",
          "Trip",
        ])
      ...
      e
        ...
        .withValueMax(220),
      e
        .enum("leakage_setting", ea.STATE_SET, ["Ignore", "Alarm", "Trip"])
        .withDescription("Leakage setting"),
      e
        .numeric("leakage_threshold", ea.STATE_SET)
        .withUnit("mA")
        .withDescription("Setup value on the device")
        ...
        .withValueMax(100),
      e
        .enum("temperature_setting", ea.STATE_SET, ["Ignore", "Alarm", "Trip"])
      ...
      e
        ...
        .withValueMax(100),
      e
        .enum("over_power_setting", ea.STATE_SET, ["Ignore", "Alarm", "Trip"])
      ...
      e
        .binary("restore_default", ea.STATE_SET, "ON", "OFF")
        .withDescription("Turn ON to restore default settings"),
    ],
    meta: {
      tuyaDatapoints: [
        [1, "energy", tuya.valueConverter.divideBy100],
  ...
  },
  {
    ...
    exposes: [
      ...
      e.co2(),
      e
        .enum("alarm_ringtone", ea.STATE_SET, ["melody_1", "melody_2", "OFF"])
        .withDescription("Ringtone of the alarm"),
      e
        .numeric("backlight_mode", ea.STATE_SET)
        .withValueMin(1)
        .withValueMax(3)
        .withValueStep(1)
        .withDescription("Backlight"),
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("Ledron", "QS-Zigbee-D06-DC", "Dimmer 12-36v", [
        "_TZ3210_hquixjeg",
      ]),
    ],
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .numeric("temperature_sensor", ea.STATE)
        .withUnit("°C")
        .withDescription("Floor temperature from external sensor"),
      e
        .deadzone_temperature()
        .withValueMin(1)
        .withValueMax(9)
    ...
    meta: {
      tuyaDatapoints: [
        ...
        [
          102,
          "running_state",
          tuya.valueConverterBasic.lookup({ idle: false, heat: true }),
        ],
        ...
        [
          116,
          "sensor_mode",
          tuya.valueConverterBasic.lookup({ IN: 0, OU: 1, AL: 2 }),
        ],
        ...
        [
          128,
          "preset",
          tuya.valueConverterBasic.lookup({ auto: 1, manual: 0, mixed: 3 }),
        ],
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .binary("eco_mode", ea.STATE_SET, "OFF", "ON")
        .withLabel("ECO mode")
        .withDescription("Default: Off"),
      e
      ...
      e
        .temperature_sensor_select(["IN", "OU", "AL"])
        .withLabel("Sensor")
        .withDescription("Choose which sensor to use. Default: AL"),
      e
      ...
      e
        .enum("factory_reset", ea.STATE_SET, ["factory reset"])
    ...
    meta: {
      tuyaDatapoints: [
        [
          1,
          "system_mode",
          tuya.valueConverterBasic.lookup({ off: false, heat: true }),
        ],
        [
          2,
          "preset",
        ...
        [
          28,
          "factory_reset",
          tuya.valueConverterBasic.lookup({ factory_reset: true }),
        ],
        [
          36,
          "running_state",
  ...
  },
  {
    ...
    exposes: [
      e.switch().setAccess("state", ea.STATE_SET),
      e
        .enum("valve_state", ea.STATE, ["close", "unknown", "open"])
        .withDescription("State of the valve"),
    ],
    meta: {
      tuyaDatapoints: [
        ...
        [
          ...
          null,
          {
            from: (v: number) => {
              ...
              const presenceState = Object.entries(lookup).find(
                (i) => i[1].valueOf() === v,
              )[0];
              return {
  ...
  },
  {
    ...
    exposes: [
      ...
      tuya.exposes
        .switchMode2()
        .withEndpoint("l1_l2")
        .withLabel("1-2 channels"),
      tuya.exposes.switch().withEndpoint("l1"),
      tuya.exposes.switch().withEndpoint("l2"),
      tuya.exposes
        .switchMode2()
        .withEndpoint("l3_l4")
        .withLabel("3-4 channels"),
      tuya.exposes.switch().withEndpoint("l3"),
      tuya.exposes.switch().withEndpoint("l4"),
      tuya.exposes
        .switchMode2()
        .withEndpoint("l5_l6")
        .withLabel("5-6 channels"),
      tuya.exposes.switch().withEndpoint("l5"),
      tuya.exposes.switch().withEndpoint("l6"),
      tuya.exposes.switchType(),
      e.power_on_behavior(["off", "on"]).withAccess(ea.STATE_SET),
  ...
  },
  {
    ...
    exposes: [
      ...
      tuya.exposes
        .switchType()
        .withEndpoint("l1_l8")
        .withLabel("Switch type 1-8 channels"),
      tuya.exposes.switch().withEndpoint("l1"),
      tuya.exposes.switch().withEndpoint("l2"),
      tuya.exposes.switch().withEndpoint("l3"),
      tuya.exposes.switch().withEndpoint("l4"),
  ...
  },
  {
    ...
    extend: [
      tuyaBase({ dp: true }),
      m.deviceEndpoints({ endpoints: { l1: 1, l2: 1, l3: 1 } }),
    ],
    exposes: [
      e.switch().withEndpoint("l1").setAccess("state", ea.STATE_SET),
      e.switch().withEndpoint("l2").setAccess("state", ea.STATE_SET),
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withDescription("detection sensitivity"),
      e
        .binary("backlight", ea.STATE_SET, "ON", "OFF")
        .withDescription("backlight"),
      e
      ...
      e
        .enum("trigger_switch", ea.STATE_SET, ["ch1", "ch2", "ch3"])
        .withDescription("Switch state reversal"),
    ],
    meta: {
      tuyaDatapoints: [
        [1, "presence", tuya.valueConverter.trueFalse1],
  ...
  },
  {
    ...
    whiteLabel: [
      tuya.whitelabel("RTX", "TS0601_RTX_DIN", "Din rail switch", [
        "_TZE200_abatw3kj",
      ]),
    ],
    exposes: [
  ...
  },
  {
    ...
    exposes: [
      e
        .binary("alarm", ea.STATE_SET, "ON", "OFF")
        .withDescription("Sound the alarm"),
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .max_temperature_limit()
        .withDescription("Maximum temperature (default: 35 ºC)")
        .withValueMin(5)
        .withValueMax(35)
        .withValueStep(0.5),
      e
    ...
    meta: {
      tuyaDatapoints: [
        [
          1,
          "system_mode",
          tuya.valueConverterBasic.lookup({ off: false, auto: true }),
        ],
        [2, "current_heating_setpoint", tuya.valueConverter.divideBy10],
        [
          4,
        ...
        [
          18,
          "open_window_active",
          tuya.valueConverterBasic.lookup({ OFF: false, ON: true }),
        ],
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withValueMax(90),
      e
        .binary("switch_type", ea.STATE_SET, "ON", "OFF")
        .withDescription("Enables/disables valve switch"),
      e
    ...
    meta: {
      tuyaDatapoints: [
        ...
        [
          112,
          "local_temperature_calibration_f",
          tuya.valueConverter.localTempCalibration1,
        ],
        [
          114,
          "valve_control_type",
          tuya.valueConverterBasic.lookup({ PID: 0, ONOFF: 1 }),
        ],
  ...
  },
  {
    ...
    exposes: [
      ...
      e.humidity(),
      e
        .binary("manual_mode", ea.STATE_SET, "Auto", "Manual")
        .withDescription("Manual = Manual or Schedule = Auto"),
    ],
    meta: {
      tuyaDatapoints: [
  ...
  },
  {
    ...
    exposes: [
      e
        .binary("valve_1", ea.STATE_SET, "ON", "OFF")
        .withDescription("Switch state"),
      e
      ...
      e
        .binary("valve_2", ea.STATE_SET, "ON", "OFF")
        .withDescription("Switch state"),
      e
    ...
    whiteLabel: [
      tuya.whitelabel("Nova Digital", "ZVL-DUAL", "Water Valve with 2 zones", [
        "_TZE284_iilebqoo",
      ]),
    ],
  },
  {
    ...
    exposes: [
      ...
      e
        .numeric("closest_target_distance", ea.STATE)
        .withDescription("the target distance away from the sensor")
        .withUnit("m"),
      e
      ...
      e
        ...
        .withUnit("m"),
      e
        .binary("restore_factory", ea.STATE_SET, "ON", "OFF")
        .withDescription("restore_factory"),
      e
        .binary("led_indicator", ea.STATE_SET, "ON", "OFF")
        .withDescription("turn on or off the led "),
    ],
    meta: {
      tuyaDatapoints: [
        [104, "presence", tuya.valueConverter.trueFalse1],
        ...
        [
          4,
          "largest_movement_detection_distance",
          tuya.valueConverter.divideBy100,
        ],
        [
          108,
          "largest_presence_detection_distance",
          tuya.valueConverter.divideBy100,
        ],
        [
          106,
          "restore_factory",
          tuya.valueConverterBasic.lookup({ ON: false, OFF: true }),
        ],
        [
          107,
          "led_indicator",
  ...
  {
    fingerprint: tuya.fingerprint("TS0225", [
      "_TZ321C_fkzihax8",
      "_TZ321C_4slreunp",
    ]),
    model: "MTD085-ZB",
    ...
    exposes: [
      ...
      e
        .enum("distance_report_mode", ea.STATE_SET, [
          "Normal",
          "Occupancy detection",
        ])
        .withDescription(
      ...
      e
        .numeric("debug_distance", ea.STATE)
        .withDescription("Real time distance to target")
        .withUnit("m"),
      e
        .numeric("debug_countdown", ea.STATE)
        .withDescription("Time before the target disappears")
        .withUnit("s"),
    ],
    meta: {
      tuyaSendCommand: "sendData",
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withDescription("Auto Mode"),
      e
        .enum("manual_mode", ea.STATE_SET, [
          "off",
          "antifrost",
          "eco",
          "comfort_-2",
      ...
      e.window_detection(),
      e
        .enum("window_opening_mode", ea.STATE_SET, [
          "off",
          "antifrost",
          "eco",
          "comfort_-2",
      ...
      e
        ...
        .withDescription(
          "The remaining " +
            "time for the function will be counted down in seconds ( 900 to 0 ) when the boost heating is activated.",
        )
        .withValueMin(0)
        .withValueMax(900),
      e
      ...
      e
        .binary("reset_consumption", ea.STATE_SET, "ON", "OFF")
        .withDescription("Reset energy consumption to zero")
        .withCategory("config"),
    ],
    meta: {
      tuyaDatapoints: [
        ...
        [
          116,
          "schedule_wednesday",
          tuya.valueConverter.PO_BOCO_ELEC_schedule(3),
        ],
        [
          117,
          "schedule_thursday",
          tuya.valueConverter.PO_BOCO_ELEC_schedule(4),
        ],
        [118, "schedule_friday", tuya.valueConverter.PO_BOCO_ELEC_schedule(5)],
        [
          119,
          "schedule_saturday",
          tuya.valueConverter.PO_BOCO_ELEC_schedule(6),
        ],
    ...
    meta: {
      tuyaDatapoints: [
        ...
        [
          19,
          "local_temperature_calibration",
          tuya.valueConverterBasic.divideBy(1),
        ],
        [
          102,
          "running_state",
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withDescription("energy")
        .withUnit("kWh")
        .withDescription("Consumed energy"),
      e
        .enum("power_outage_memory", ea.STATE_SET, ["on", "off", "restore"])
        .withDescription("Recover state after power outage"),
      e
        .enum("fault", ea.STATE, [
          "clear",
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .enum("fault", ea.STATE, ["Normal", "None", "Fault"])
        .withDescription("Motor Fault"),
      e
        .numeric("countdown", ea.STATE_SET)
        .withValueMin(10)
        .withValueMax(90)
        .withUnit("s")
        .withDescription("Motor timeout"),
      e
        .enum("motor_direction", ea.STATE_SET, ["Left Side", "Right Side"])
        .withDescription("Pusher install side"),
      e
        .enum("mode", ea.STATE_SET, ["Enable", "Disable"])
        .withDescription("Slow stop mode"),
      e
        .enum("fixed_window_sash", ea.STATE_SET, ["Up", "Down"])
        .withDescription("Button position"),
      e
        .enum("window_detection", ea.STATE, ["Opened", "Closed", "Pending"])
        .withDescription("Window detection status"),
    ],
    meta: {
      tuyaSendCommand: "sendData",
      tuyaDatapoints: [
        ...
        [
          102,
          "state",
          tuya.valueConverterBasic.lookup({ OPEN: 0, CLOSE: 1, STOP: 2 }),
        ],
        ...
        [
          106,
          "manual_mode",
          tuya.valueConverterBasic.lookup({ Enable: 0, Disable: 1 }),
        ],
        [
          107,
          "fault",
          tuya.valueConverterBasic.lookup({ Normal: 0, None: 1, Fault: 2 }),
        ],
        [108, "countdown", tuya.valueConverter.raw],
        [
          109,
          "motor_direction",
          tuya.valueConverterBasic.lookup({ "Left Side": 1, "Right Side": 0 }),
        ],
        [
          110,
          "mode",
          tuya.valueConverterBasic.lookup({ Enable: 1, Disable: 0 }),
        ],
        [
          112,
          "fixed_window_sash",
          tuya.valueConverterBasic.lookup({ Up: 1, Down: 0 }),
        ],
        [
          114,
          "window_detection",
          tuya.valueConverterBasic.lookup({ Opened: 0, Closed: 1, Pending: 2 }),
        ],
      ],
    },
  },
  ...
  },
  {
    ...
    exposes: [
      e
        .enum("rainwater", ea.STATE, ["none", "raining"])
        .withDescription("Sensor rainwater status"),
  ...
  },
  {
    ...
    exposes: [
      exposes
        .enum("presence_state", exposes.access.STATE, [
          "none",
          "motion",
          "stationary",
        ])
  ...
  },
  {
    ...
    exposes: [
      e.cover_position().setAccess("position", ea.STATE_SET),
      e
        .enum("motor_direction", ea.STATE_SET, ["forward", "back"])
        .withDescription("Motor direction"),
      e
        .enum("border", ea.STATE_SET, [
          "up",
          "down",
          "up_delete",
          "down_delete",
